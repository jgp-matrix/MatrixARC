...
**#2a — LIVE LEASE INDICATOR (the PRIMARY UX defect): persistent, reactive, holder-named banner + re-arm on holder change.** Today the ONLY lease UI is the one-shot open-time modal (`leaseModal`, guarded by `_leaseModalShownRef`) — a user already IN the project when the lease flips gets no standing indicator → silent read-only.
