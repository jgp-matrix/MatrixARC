...
- **Per-session tab keying (ruling #8):** `editingTabId` compare is CLIENT-enforced (rules are per-uid — same user's 2nd tab is made read-only client-side; the cross-USER guarantee is server-enforced). Closes the self-clobber residual without Phase B's merge (Phase B stays shelved).
