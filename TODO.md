...
- **G001 — "Suppress the Allow-Once prompt on cross-session sends"** [Verified — NOT fixable; accepted limitation] — goal was to stop Jon being desk-tethered to approve the per-send "Allow Once" `send_message` prompt. *(source: Jon 2026-07-02, via Freddy; captured FEAT, reclassified G — dev-tooling/infra.)*
  ...
  - **Disposition — ACCEPTED LIMITATION, no fix.** The per-send prompt is unavoidable by design; there is no remote-approval and no allowlist/mode escape. Mitigations: keep sends batched; for unattended stretches use the **repo-commit handoff bus**; the **stuck-on-Allow-block monitor stays permanently useful** (the prompt is here to stay). The committed allow rule (`a1e786d3`) is a NO-OP → reverted. SESSION-STATE/CLAUDE.md "correction" reverted (the original hardcoded wording stands).
