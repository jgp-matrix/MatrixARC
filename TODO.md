...
- **F035 — "Move + resize a placed markup shape"** [OPEN · MED-HIGH · L] — shapes render in a `pointerEvents:none` SVG (`:31025`) — only NOTES drag today (`:30966`). Add drag + per-type resize handles (rect corners / circle radius / triangle verts / line ends). Highest user value of the cluster, biggest build. Rides merge-guarded `reviewShapes` (no parallel storage). Builds on B051. *(engineer feedback, triaged 2026-07-22.)*
