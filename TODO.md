...
- **B047 — "ProjectTile owner/EDITING name overflows the tile at narrow width"** [ON TEST V.025 · MED · ✅ shipped-to-test — awaiting Jon prod OK] — Jon (2026-07-22, screenshot at min tile width): Row-1 right-side owner name / "<FIRST> EDITING" ran OFF the tile. FIXED (worktree `adaf2356` → master `5de11e66`): Row 1 `overflow:hidden`; owner name shrinkable+ellipsis, right-justified; EDITING indicator `whiteSpace:normal`+`maxWidth:64` so "RYAN EDITING" wraps to 2 lines. Presentation-only. On matrix-arc-test V.025. *(source: Jon 2026-07-22.)*
