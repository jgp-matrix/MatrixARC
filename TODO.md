...
> **B065 BUILD-PLAN ready → `docs/B065-DURABLE-BINDING-PLAN.md`** (Coach): persist `panel.bcTaskNo` + `row.bcLineNo` (read back from 2xx), shared `resolveBcBindings` (= F070's read path), resolve-don't-compute + fail-loud self-heal, part-identity guard on DELETE (closes the wrong-line-overwrite money risk), convert ~27 call sites, fold relink off-by-one, lazy migration with an **ambiguity guard for the duplicate-panel-name case** (bind-unambiguous-only + flag `bcBindingUnresolved` + require Jon reconcile — never auto-bind-then-destroy). 5 open questions for Jon (discovery key, migration strictness, reorder-renumber tradeoff, part# uniqueness, SystemId-vs-Line_No). Next build after B067 ships; awaiting Jon go on the plan + its open questions.
