...
### 🐛 Bugs (B###)
- **B048 — "To-Do rail Sales Pipeline data lags a few seconds on page reload"** [OPEN · MED · 🔍 diagnosing] — Jon (2026-07-22): on reload the right-side To-Do pane takes a few seconds to populate its Sales Pipeline pills. Coach diagnosing the data-dependency chain (projects listener latency vs. late `_arcSalespersonCache` / permissions resolve vs. missing loading state). Read-only diagnose → fix menu, then scope. *(source: Jon 2026-07-22.)*
