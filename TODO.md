...
- **B006 — "Scan Results 'concern' window is persistent with no relevant data"** [Backlog] — the "Scan Results" concern window shows persistently but carries no relevant data (UI clutter). FIX (Jon 2026-07-06): add a **"Dismiss"** button so the user can get rid of it — user-dismissable (NOT an auto-hide); re-surfaceable later if/when it has relevant data. Needs a quick locate (which component/panel) + a dismiss-persistence decision (session vs per-project). *(source: Jon 2026-07-06.)*
