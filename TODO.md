...
- **F040 — "Shape notes render beside their shape, movable, with a leader line to the shape"** [OPEN · MED-HIGH · M — 🔍 scoping w/ F035] — Jon (2026-07-22, testing B051 on prod): a shape's note should render as a label positioned NEXT TO the shape, be independently MOVABLE (drag the note), and draw a **leader line** connecting the note back to its shape. Needs a data model (add-only per Data Retention): note position/offset + shape linkage + leader anchor; existing shape-notes (`shape.note` text, ~:31073/:31245) default to beside-shape with no migration. Pairs with F035 (same overlay/interaction model). *(source: Jon 2026-07-22.)*
- **F035 — "Move + resize a placed markup shape"** [OPEN · MED-HIGH · L — 🔍 scoping (Jon prioritized 2026-07-22: "once placed a shape can't be moved — need to move them after placing")] — shapes render in a `pointerEvents:none` SVG (`:31025`) — only NOTES drag today (`:30966`). Add drag + per-type resize handles (rect corners / circle radius / triangle verts / line ends). Highest user value of the cluster, biggest build. Rides merge-guarded `reviewShapes` (no parallel storage). Builds on B051. *(engineer feedback, triaged 2026-07-22.)*
