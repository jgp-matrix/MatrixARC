...
> _Features:_
> - **F078 [FEAT · Jon 2026-07-31]** — **API auto-pricing on extraction** ("should happen on extraction too — verify this"). Cross-ref **F075**. ✅ VERIFIED (2026-07-31): API (Mouser/DigiKey) pricing does NOT auto-fire on extraction — it's **manual-only** via the "Get Prices" button (`runApiPricingOnPanel`, 31171; UI at 32271/32277). Extraction auto-runs `runPricingOnPanel` (28816) = BC matching/pricing + scraper phases (scrapers currently off). So this IS a real build (add API to the post-extract pipeline). ⚠ COST CAVEAT for Jon: DigiKey/Mouser are paid calls — auto-firing on every extraction/row/project has real cost (likely why it's manual). Decide: auto for all, or gated (e.g. only rows BC can't price)?
...
> - **[BUG · MED] ⏳ FIX BUILT — on Test V.073 (branch `claude/ip66-partnum-recovery`), awaiting Jon re-extract verify.** **IP66 / 1200 wrong Part#** — model writes a rating (`IP66`) / dimension (`1200`) from the DESCRIPTION column into partNumber instead of the real Part#-column value (`640014405`/`8660025`). REAL rows, bad Part# (NOT a delete). DETERMINISTIC recovery `_recoverMisreadPartNumbers` (app.jsx:17300+): text-layer PDFs — join misread model row → PDF text-layer row by description, swap in the catalog token from the inferred Part# column, stash old value in notes, flag low-confidence. Headless regression recovers both + leaves controls untouched. Coach APPROVE-WITH-NITS (x-distance ceiling landed). **Live root-cause found + fixed:** recovery read `window._pdfjs` without `await window.pdfjsReady()` → silent no-op on re-extract (headless missed it); fixed commit `177d037c`. Scanned pages (no text layer) stay review-flagged.
> - **[BUG · MED] ✅ RESOLVED v1.24.60 (2026-07-30).** `?PN`/`~PN` validity badges going dark was a transient Haiku 529 overload; added Debug-Logs surfacing on both `verifyPartNumbers` failure paths + a visible "🔎 Verify Part #s" toolbar button (`runVerifyPn`). Jon confirmed "PN badges are showing."
> - **[FEATURE · new · Jon 2026-07-30] ⏳ SCOPE READY — `docs/F-secondary-vendor-rfq-scope.md`, awaiting Jon decision.** At RFQ send, let the user opt to also request quotes from each item's SECONDARY suppliers. Recommend Option A (global toggle, BC PurchasePrice fan-out). Decisions needed: approve Option A? secondary = comparison-only vs auto-applicable? request lead-time or price-only?
> - **[Note]** F077 Vendor Sync UI is LIVE on prod (v1.24.61, `VendorNumberConfig`). Remaining = Jon sets DigiKey `V00196` / Mouser `V00304` via Settings → Vendor Sync (data-config, not a build) to activate the F075 optional API→BC writeback.
> - **✅ SHIPPED PROD v1.24.61 (2026-07-30):** RFQ WYSIWYG vendor routing — RFQs route to the vendor shown on the BOM row (primary OR user-selected secondary); removed the crossed-item re-resolve that mis-routed via a stale bcNo (PRJ402143 / SCE-60EL6018LPPL → Royal). Jon Test-verified (routes to Intermountain). Coach-approved.

> **🆕 NEW 2026-07-28 (Jon, via live presence issue) — B069 [BUG · LOW-MED · TABLED]:** **Sign Out from inside a project doesn't release the project immediately** → a teammate's presence ("X is editing/viewing") lingers up to **90s**. ROOT: the Sign Out handler (`src/app.jsx:52818`) clears caches + nulls the BC token then calls `fbAuth.signOut()` — it does NOT delete the `companies/{cid}/projectPresence/{projectId}_{uid}` doc. Presence delete happens ONLY in the ProjectView unmount-cleanup (`:40409`), which runs AFTER `signOut()` revoked the Firebase auth token → delete is **permission-denied** → doc orphaned → the 90s stale filter (`:40399`) is what finally clears it (heartbeat is 30s; window 90s). **FIX:** in the Sign Out handler, `await` a delete of the active project's presence doc BEFORE `fbAuth.signOut()` (auth still valid) → immediate release; keep the 90s window as the fallback for hard browser-closes (where `beforeunload`'s async delete can't reliably flush — inherent web limitation). Small/isolated, LOW risk. **TABLED** with all feature work pending the BC-sandbox migration (Jon 2026-07-28).

> **✅ SHIPPED PROD 2026-07-27 — BC-INTEGRITY BATCH #2 (v1.24.42 + prod functions `a6e7068c`):** **F071** [FEATURE·HIGH] BC commit-gate — 6 Tier-A money-path hard-blocks (Send Quote · PO submit · New BC project · Item-Card+PurchasePrice push · BC Item Browser "Create in BC" · Send Quoted BOM) when BC down/structural-fault/unverified; SSOT `_bcCommitBlocked()`+`useBcCommitGate()`, gate on sustained-RED only (8s grace, amber self-heals), reads/edits/print stay usable; Coach APPROVE-WITH-NITS, **Freddy live-verified end-to-end Test V.061** (fault→block→recover; grace-less token-null recovery window = Coach's known NIT, accepted). Jon-ruled both flagged sites HARD-BLOCK; Tier-B enqueue-routing = deferred follow-up. · **B016 (safe half)** [BUG·HIGH] debug-write throttle (Fix D) + on-open sync gate (Fix C) — de-amplify the console.error→debugLogs.add write storm that stalls sessions; Fix A/B DEFERRED (need Jon save-latency/offline answers). · **B064 (aggregator)** [BUG·HIGH] server-side `onIssueReported` extension → ONE deduped Teams+push admin alert on a company-wide `bcStructuralFault` storm (threshold 5/24h, runTransaction dedupe, skips test); Coach APPROVE-WITH-NITS. Docs: `docs/F071-*`, `docs/B016-B012-*`, `docs/B064-*`.
> **🆕 NEW 2026-07-27 (Jon, vendor/item architecture) — F072 + F073 SCOPED (plans written, not built):** **F072** [FEATURE·money-path] secondary-vendor/supplier management — a NEW SECTION in the BC Item Browser (below Part#, above Drawing Reference; modal grows H+W) listing all alternatives + MFRs + suppliers; primary=Item Card `Vendor_No` (already auto-selected, untouched), secondaries=BC Purchase Price records (`bcFetchPurchasePricesMultiVendor` already retains them); ARC-side-only v1, persist choice to new `config/vendorPreferences`; shares surface w/ F010. `docs/F072-SECONDARY-VENDOR-PLAN.md`. **F073** [FEATURE·BC-job-structure] auto-create per-panel Engineering/Commissioning/Design tasks — split today's combined `20N20 "Engineering Design"`; Coach recommends **20N20/21/22** (Jon's `20N30` sketch COLLIDES with the ECO slot range 20N30–39); v1 empty Posting skeletons; ⚠ confirm these per-panel internal-cost tasks are IN ADDITION to the sell-side Service Cards (50100/50300); reserve slots in B065's binding map. `docs/F073-ENG-COMM-DESIGN-LINES-PLAN.md`.
> **✅ RESOLVED 2026-07-27 (v1.24.43, `09056fb1`; Jon-verified "LT shows" on Test V.062 → prod):** **B062 [BUG · money-path-adjacent]** — BC Item Browser lead-time flow. The Browser now SHOWS lead times (**bc_vendor**/ItemVendorCatalog authoritative per Q1, lazy-fetch on visible rows per Q2, 12-row cap removed), **USE carries the LT** to the BOM row (overwrite-manual per Q3), and a **same-part re-select full-syncs LT across Lines** (Q4, via `_fullCrossLinePatch`). Was BUILT pre-emergency on `claude/b062-leadtime-flow` (`004175fa`) but never reviewed/shipped; this session merged-forward v1.24.38→.42 (clean auto-merge, 0 code conflicts), Coach APPROVE-WITH-NITS (composes cleanly with F068 `commitBcItem` / F071 gate / B063 search — true-cross path byte-unchanged; 2 non-blocking nits: N1 unobserve tidy, N2 fast-clicker-before-lazy-LT edge). Plan: `docs/B062-LEADTIME-PLAN.md`. **F072 (secondary-vendor Alternates) now building on top** — plan `docs/F072-SECONDARY-VENDOR-PLAN.md`.
> **⏳ ON TEST V.063 — awaiting Jon verify (2026-07-27, away-mode autonomous):** **F072** [FEATURE · money-path] secondary-vendor Alternates BUILT (Marc, +291 lines `src/app.jsx`) → Coach **APPROVE-WITH-NITS** (1 must-fix folded: free-text alt supplier clears stale `bcVendorNo` to avoid a name/number mismatch feeding RFQ/PO routing; + `price>0` adopt-guard; other 3 notes ruled acceptable-v1) → committed `aee396eb` on `claude/f072-alternates` → **Test V.063** (base v1.24.43). Read-only self-verified: app loads clean (no crash), `loadItemAlternates`/`saveItemAlternates` live. **HELD at prod** — apply-path verify mutates a BOM row + the `itemAlternates` DB (needs Jon's live verify on disposable data). Verify steps in STATUS.md "FOR JON". Then merge `claude/f072-alternates` → master + `deploy.sh`. **v1 deferred/re-enable-prereq:** wire `vendorSource:"manual-secondary"` into vendor-auto-assign when pricing is re-enabled (mirrors the `priceSource:"manual"` skip).
> **✅ RESOLVED — F072 SHIPPED PROD v1.24.44 (`36eb6d9e`, 2026-07-28; Jon-verified on Test V.064 w/ a BC-write Network trace; merged `claude/f072-alternates`→master `7506a03b`):** Jon's Test-V.063 verification reframed F072 — supplier must be a BC vendor (dropdown + Create-New), and Save must WRITE Purchase Price + Item Vendor Catalog to BC (not ARC-side). Re-scoped (Coach) + **rebuilt** (Marc, +258/-133, `config/itemAlternates` DROPPED — BC is single source of truth) → Coach money-path review **APPROVE-quality + M1 folded** (reject $0 price → no junk $0 PP to BC; all 3 write paths F071 HARD-BLOCK when BC down; no silent success) → committed `6a867470`, **Test V.064** (base v1.24.43). Read-only self-verified (clean load, itemAlternates gone, write helpers wired); **Jon verified the BC writes live on Test V.064 → shipped prod v1.24.44.** Decisions LOCKED (Jon 2026-07-28): full BC-write · select-secondary also re-writes · hard-block when BC down · fetch base UoM · drop itemAlternates · re-write always. `docs/F072-SECONDARY-VENDOR-PLAN.md` (BC-write re-scope on master).
...
> **✅ SHIPPED PROD 2026-07-24 — BIG BATCH (v1.24.34 → v1.24.36), all Jon-verified.** **F065** cross-Line Part# propagation + **full-sync** [Update all] (price+LT+vendor in one click) + Bug A (override manual) / Bug B (index self-heals on open) / Bug C (confidence pill clears) — `v1.24.35`+`v1.24.36`. **B060** `applyConfirmedPrice` promote-on-success (kills the BC-circle flicker) — `v1.24.35`. **B058** BC "not in BC" circle now membership-driven (price-independent) — `v1.24.34`. **B061** Ext$ colgroup width (64→108) + `$`-left format — `v1.24.35`/`.36`. **F066** Duct/DinRail/DuctCover RED only on $0.00/qty=0 — staleness + estimated-LT + LT-RFQ all exempt (`_isReorderCommodity`, Part#-match) — `v1.24.36`. **F067** 3-min auto-approve for cross-Line updates (banner+Cancel) — `v1.24.36`. **G019** Engineering-Questions hidden behind flag — `v1.24.33`. **G020** Get-New-Pricing hidden (RFQ-only) — `v1.24.36`. **G021** PRJ# in Panel Summary header — `v1.24.36`. Plus **F057–F064** display/dashboard batch. Coach-reviewed throughout. **B032** → RESOLVED (stale EMERGENCY tag; fix shipped v1.23.12, verified present in prod). Analyses/plans: `docs/F065-*`, `docs/F068-*`, `docs/B060-*`, `docs/PRJ402119-PRICING-INCIDENT.md`.
> **✅ SHIPPED PROD 2026-07-27 (v1.24.37, `fce87edc`):** **F068** — propagate the CROSS across Lines (full-sync): crossing a part A→B on one Line prompts to cross the same part on the other Lines to B (part#+bcNo+price+LT+vendor). Ships WITH the **near-miss** enhancement — a Line whose part# differs only by a manufacturer prefix (e.g. `1492-D2Cxxx` vs `AB 1492-D2Cxxx`) now surfaces in the prompt in a separate "formatted differently" section with a **default-OFF per-row opt-in** (loose-match via curated `BC_MFR_MAP`/`_MFR_ALIASES` tokens; `_samePartKey`/`normPart` untouched). Coach code-review APPROVE-WITH-NITS (N1 folded). **Verification:** exact-match path LIVE-verified on Test V.056 (CR5362→5603049 propagated 4/4 Lines w/ price+LT+vendor); near-miss path Coach-approved + code-verified but **NOT live-verified** (can't hand-input an originally-scanned prefixed part# to trigger it — Jon accepted). Docs: `docs/F068-BUILD-PLAN.md`, `docs/F068-NEARMISS-PLAN.md`, `docs/F068-CROSSED-SUPERSEDED-ANALYSIS.md`.
> **⏳ FOLLOW-UPS from F068 near-miss (NEW 2026-07-27):** **(a) F065 mfr-prefix gap** — F065's auto price/LT propagation (`buildCrossLineDuplicates`, exact `_samePartKey` keys) still treats a mfr-prefix-inconsistent Line as a different part → skips it silently (same root as the F068 Line-1 miss). Jon scoped the near-miss fix to the F068 prompt only; extending loose-match to F065 auto-propagation is a separate decision. **(b) N2 (LOW)** — `_getMfrTokenSet` memoizes the whole build in one try/catch; a future malformed `BC_MFR_MAP` entry lacking `.terms` would permanently memoize a partial set. No impact today; add a per-entry guard if that table's shape ever changes.
> **✅ RESOLVED 2026-07-27 (v1.24.38, `e1d28505`; Jon-verified on Test V.057 → prod):** **B063 [BUG · LOW]** — BC Item Browser search now case-insensitive for part numbers. Fix = uppercase the search token for number-collated BC fields (`No`/`Vendor_Item_No`/`Common_Item_No`/`Manufacturer_Code`) in both the `field=both` fan-out (`:5062`) and `_andOn('number')` (`:5036`) via `_isBcNumberField`; descriptions left untouched (already case-insensitive); `tolower()` avoided (BC-SaaS-rejected). Original ask below.
> **🆕 NEW 2026-07-27 (Jon, via Intake):** **B063 [BUG · LOW · not money-path]** — make the **BC Item Browser search case-insensitive**. `bcSearchItems` (`src/app.jsx:5011`) builds the server-side BC OData primary-token query as `contains(${field},'${primary}')` (`:5062`) with the ORIGINAL-case token → OData `contains()` is case-sensitive, so lower/mixed-case searches miss items. The client-side secondary-token filter already lowercases (`:5058+`) — only the server primary-token fetch is case-sensitive. **⚠ CORRECTION (2026-07-27, Marc trace):** the `tolower()` fix is **KNOWN-BROKEN** — already tried + reverted in **v1.19.806** (BC SaaS 400s on `tolower()` in `$filter`: silent parser failure → "Blocked field" error on *No*). Comment lives at `:5033`. Also per that decision, BC `contains()` is **already case-insensitive on DESCRIPTION fields** (tested: lowercase 'switch' → 33 items). **Jon repro (2026-07-27):** lowercase **PART NUMBER** search → **returns nothing** → the item-number fields (`No`/`Vendor_Item_No`/`Common_Item_No`/`Manufacturer_Code`) are **case-sensitively collated** (part#s stored UPPERCASE). **New fix direction:** uppercase the server-side `primary` token used in the `contains()` fan-out (`:5062`) — `const primary=literalTokens[primaryIdx].toUpperCase()` (or per-field uppercase for number fields). Safe: descriptions are already case-insensitive so uppercasing keyword tokens still matches; the client-side secondary-token filter already lowercases (`:5058`). **Pending live confirm:** UPPERCASE part# search returns the item (⇒ uppercasing the token is the fix). Quick, isolated, not money-path.
> **🚨 EMERGENCY 2026-07-27 — "Ryan constantly disconnecting from BC" → live diagnostic (`docs/RYAN-BC-DISCONNECT-DIAGNOSTIC-2026-07-27.md`).** Root cause: `Project_Planning_Lines_Excel` OData web service 404s ("Resource not found for the segment") in the BC SANDBOX `MATR_SndBx_01152026` — **CHRONIC since ~June 22 (522 hits, ~daily), affecting BOTH jon@ (63) and Ryan (32)**, silent because 404≠pill-red. NOT MSAL/Mode-A (0 token errors), NOT an env mismatch (all users match Settings env), NOT an ARC code change. BC-side (orphaned web-service after a likely sandbox refresh). **Jon (BC admin) verifying/republishing the table.** Three ARC work items (Jon: "all of the above"):
>   - **B064 [BUG · HIGH]** — BC failures that bypass the honest pill (404 "segment not found" on a discovered web service + B013-G1 raw-fetch 401 family) must SURFACE (admin signal + persistently-failing-endpoint alert), not fail silently for 5 weeks. Extends **B013**. Coach to scope.
>   - **B016/B012 (write-exhaustion amplifier)** — failing BC ops churn writes → `resource-exhausted`/max-backoff → session stalls (Ryan's visible "disconnect"; also jon@/Noah). Harden per B016 (await/confirm per mutation + churn reduction). Coach to scope.
>   - **F069 [FEATURE · guard]** — hard-block any BC op when user's/project's env ≠ Settings env (build on `_bcEnvMismatched` + per-project `bcEnv`). Defense-in-depth (Jon directive: "no user should EVER be in a BC env ≠ Settings"); not the cause here. Coach to scope.
> **★ ROOT CAUSE CORRECTED (2026-07-27, live BC OData probes) → B065 [BUG · HIGH]:** the emergency is NOT the web service (published + resolves; keyed GET on a real record = 200 — Jon confirmed) and NOT an env mismatch. **ARC requests PRJ402141 planning lines under `Job_Task_No=20510`, which DOES NOT EXIST** — the real tasks are `20110/20210/20310/20410/20610` (121 lines; 20510=0 lines, and 20510 is skipped in favor of 20610). So ARC's stored/computed panel→BC task-number mapping is **stale/wrong** (BC tasks renumbered/shifted, likely ~June 22) → every planning keyed request 404s → silent (404≠pill; `bcGatedFetch` is 404-blind) → write-churn stalls the session. **B065 fix:** ARC must re-resolve the actual `Job_Task_No` per panel from BC (read the job's real tasks; don't assume sequential), + fail loudly on task-not-found (ties to B064). Needs a Jon BC-domain confirm: were PRJ402141's tasks renumbered (no 20510, added 20610)? Coach to scope B065.
> **★ B065 DELETE-TRACE (2026-07-27):** Ryan did NOT delete anything (created w/1 line, added 5 → ARC has 6; BC has 5). Debug logs show **19× `SAVE BLOCKED: would reduce panels from N→M`** (07-20 + 07-25, all Ryan) = a **spurious panel-REDUCTION write bug** (B016 write-race class) that ARC's guard blocked on the Firestore side but **leaked to BC as the silent deletion of task 20510** (no BC-delete error logged). Project has **duplicate panel names** (2×"Panel 2", 2×"Panel 4") + **no stored per-panel task#** → ARC computes Job_Task_No by position → broken panel-add flow both drives the reductions AND misaligns the task numbers. **B065 fix spans B016 (kill the spurious reduction) + a durable panel↔BC-task binding + 404 self-heal.** Root-cause bug is upstream in B016.
> **🆕 NEW 2026-07-27 (Jon obs, via emergency) — B066 [BUG · investigate]:** scanned drawings not appearing in BC for PRJ402141. ARC-side extraction ran fine (addFiles/extractBomBatch/extractBomPage all logged OK) but **ZERO BC document-attachment ops logged** (`bcAttachPdfToJob`/`documentAttachments`). Verify directly vs BC (`projects(jobId)/documentAttachments`); if empty → BC attach-on-scan not firing / silent-fail (same family as B064 silent BC failures). Coach to scope after Jon confirms via BC. **✅ B066 CONFIRMED (Jon 2026-07-27): manual "Upload To BC" works; attach-on-EXTRACTION never wired** (`_autoSyncBcDrawings` only re-uploads on rev, gated on a flag only the manual button sets) — audit C3.
> **★★ ARC↔BC INTEGRITY AUDIT COMPLETE (2026-07-27, 6 Coach lanes) → `docs/BC-INTEGRITY-AUDIT-2026-07-27.md`.** Two root themes: (1) **positional bindings, no durable link** — task#/Line# computed by array index everywhere, so any add/delete/reorder/BC-renumber silently mis-targets (highest risk: a BOM-row shift PATCHes the WRONG BC line + deletes the tail = wrong part ordered / dropped, silent); (2) **silent failure in 3 layers** (404-blind gate → write-fn swallows → caller fire-and-forget that stamps "synced" anyway). New/expanded items:
>   - **B067 [BUG · HIGH · NEW]** — on-open + pre-print sync stamp `bomSyncHash` WITHOUT checking `result.failed` (`:40272`/`:42075`) → a failed push is marked "synced," never retries = the 5-week masker of Ryan's 404s. Fix: gate hash-write on `result.failed.length===0` (manual path `:27133` already does).
>   - **F070 [FEATURE · NEW]** — continuous ARC↔BC mismatch DETECTOR (reconciliation). ~80% exists in the write path; factor into a read-only comparator, match on STABLE keys (part# within task, not position), ~2 GETs/project, send-time gate + on-open + manual verify, second data-integrity axis vs the connection pill, report-first + split auto-heal. Persisting the resolved binding = doubles as the B065 fix. Jon's centerpiece ask.
>   - **B065 (expand)** durable panel↔task + row↔line binding + 404 self-heal + **relink off-by-one `:40420`** + positional-`Line_No` wrong-line-overwrite. **B064 (expand)** 3-layer surfacing incl. auto-sync-check-failed + skeleton-line `_row` + DELETE else-branch + queue-drop + all-status recording. **B016 (expand)** `addPanel` stale-closure race (dup names + task-skip = why 20510 never created) + guard-Firestore-only + debug-write amplifier.
>   - **Prioritized roadmap** in the audit doc: P0 unblock+B065 · P1 B067+B064 (stop silent masking) · P2 B064-surface+F070-detector+B016 · P3 F069+B066. Always-on sweep needs app-only BC creds (today = client-MSAL only). **Awaiting Jon: roadmap sequencing + 5 detector open-questions (in the audit doc).**
> **2026-07-27 build progress:** **B067 BUILT** (branch `claude/b067-sync-hash-honest`, Coach APPROVE-WITH-NITS) — on-open + pre-print now stamp `bomSyncHash` only when `result.failed.length===0`; churn-risk ruled out (sync fn always returns `{failed:[]}` or throws→caught). **✅ RESOLVED — SHIPPED PROD v1.24.39 (`f51adbd7`, Jon-verified on Test V.058 → prod).** The honest sync-hash is live: a failed/partial BC sync no longer records "synced," so a divergence like Ryan's can't stay masked. (Remaining nit N1 = surface the failure in UI, folded into B064.) Coach nits: **N1** (auto/pre-print sync failures still only `console.warn`, no UI signal — fold into **B064** surfacing) · **N2 → B068 [BUG · LOW · NEW · pre-existing]** `computePanelBomHash` (:9600) hashes `ecoTag` rows but `bcSyncPanelPlanningLines` (:3944) excludes them → at direct-call sites a panel's hash can be stamped covering ECO rows that path never pushed (minor honest-hash gap). **Jon must still pick the Ryan immediate-unblock route (A nudge-hash / B direct-create task 20510) — B067 prevents FUTURE masking but does NOT un-mask PRJ402141's already-false-synced hash.**
> **B065 BUILD-PLAN ready → `docs/B065-DURABLE-BINDING-PLAN.md`** (Coach): persist `panel.bcTaskNo` + `row.bcLineNo` (read back from 2xx), shared `resolveBcBindings` (= F070's read path), resolve-don't-compute + fail-loud self-heal, part-identity guard on DELETE (closes the wrong-line-overwrite money risk), convert ~27 call sites, fold relink off-by-one, lazy migration with an **ambiguity guard for the duplicate-panel-name case** (bind-unambiguous-only + flag `bcBindingUnresolved` + require Jon reconcile — never auto-bind-then-destroy). 5 open questions for Jon (discovery key, migration strictness, reorder-renumber tradeoff, part# uniqueness, SystemId-vs-Line_No). **★ SPLIT (2026-07-27):** **Phase 1 FOUNDATION** — DELETE part-identity guard (money-path, stops wrong-part-ordered/dropped-part) + relink off-by-one fix + additive-dormant binding scaffolding — Coach APPROVE-WITH-NITS, **✅ SHIPPED PROD v1.24.40 (`e5b24186`, Jon-verified Test V.059 → prod).**
> **🆕 2026-07-27 — B064 design LOCKED + F071 NEW (Jon):** **B064 [surfacing]** — build now (`docs/B064-*`): `_isStructural404` fault recorder + `_bcEndpointFaults` map + **amber "BC sync degraded — endpoint down" chip** (distinct from the connection pill) + **server-side company-wide aggregator admin alert** (Cloud Function, fires once when an endpoint stays broken ≥threshold/24h across users — the thing that catches a multi-week silence) + route the **planning/patch cluster** raw-fetch helpers through the recorder (defer admin-tool/boot-cache reads). Must expose a queryable fault/health predicate for F071 to consume. NOTE: B067 (hash-honesty) + B065-foundation (DELETE-failure arm) already covered those parts of B064's original scope. **F071 [FEATURE · NEW · commit-gate] (Jon: "if BC not connected, no work should continue" → scoped to block-commits-allow-queued-editing):** hard-block money-path COMMIT actions (Send Quote / create PO / BC planning-line sync) when BC is disconnected (401) OR an endpoint is broken (structural 404, B064 signal) OR unverified; ALLOW local BOM editing (queues via offline-queue, syncs on reconnect); DEBOUNCE so a transient blip doesn't lock out. Coach to scope (triggers: 401 vs structural-404 vs transient; coexist w/ offline-queue + RFQ-only + owner-priority lock; exact list of gated commit actions; blocking UX). Disruption-sensitive → careful scope before build.
> **2026-07-27 BC-roadmap status:** **B064** BUILT + Coach CHANGES-REQUIRED fixed (excluded keyed GETs from fault-recording — the false "endpoint down" chip / false F071 commit-block on not-yet-synced projects; wrong-task detection stays B065's job) → **✅ SHIPPED PROD v1.24.41 (`8395c44e`, Freddy-verified live on Test V.060: healthy synced projects fire all planning-line patches successfully → no false chip/pill-green; keyed-exclusion Coach+unit verified → Jon merge).** **F071 decisions LOCKED** → `docs/F071-COMMIT-GATE-PLAN.md` (hard-block: Send Quote/PO/new-project/item-price-push; enqueue: sync/patches/header/lead-times; 8s grace; portal local-save always commits; RFQ send-allowed/accept-gated — 1 reconcile at build). **F070 decisions LOCKED** (queue behind B064/F071 per Jon): send-time = soft-warn on field-diffs / HARD-block on structural mismatch · auto-heal = additive-auto / destructive-confirm · sweep = client staleness-flag now, app-only BC creds later · (+ part#-tiebreak & persist-binding already settled via B065). Design = detector section of `docs/BC-INTEGRITY-AUDIT-2026-07-27.md`. **Pipeline: B064 (Test→prod) → F071 → F070 → B065 Phase 2.** **Phase 2 DEFERRED** (resolve-don't-compute read-path + lazy migration + ~27 call sites) = tested follow-up needing live-BC validation on a ≥3-panel bound project. **4 Coach must-fix-before-Phase-2 findings recorded in `docs/B065-DURABLE-BINDING-PLAN.md`** (resolver Description-discriminator, cross-task Line_No collision, relink clear-order, dead code). All 4 in dormant code today (harmless).
> **✅ ALL 3 HARDENING SCOPES COMPLETE (Coach, 2026-07-27) — build-ready plans + open questions for Jon:** **B064** → `docs/B064-SURFACE-BC-FAILURES-PLAN.md` (key: `bcGatedFetch` is 404-blind :534 → planning 404s silent even when gated; B013-G1 already shipped the 401 half; needs a distinct amber "endpoint degraded" signal + admin alert). **B016/B012** → `docs/B016-B012-WRITE-EXHAUSTION-PLAN.md` (key self-amplifier: each `console.error`→a `debugLogs.add()` write onto the failing stream; fixes D debug-throttle/C on-open-gate/B coalescing-governor/A await-confirm). **F069** → `docs/F069-ENV-MATCH-GUARD-PLAN.md` (registry + `_assertBcProjectEnv` at ~18 write fns; fail-closed writes, soft-skip reads). All await Jon review + per-item open-question answers before build.
> **⏳ OPEN from the 2026-07-24 session:** **B062** [OPEN · MED · 🕐 TABLED] — BC Item Browser shows **no lead time** for any item (never calls `bcGetItemLeadTimeDays` :4700 per row) + USE can't carry it + **re-select-doesn't-push-LT** (the `commitBcItem` F065 fire :28653 is price-only per Coach full-sync-review F2 → for a SAME-part re-select it should full-sync the LT, price-only only for a true cross). Do as ONE lead-time-flow pass. **B059** [OPEN · LOW · parked] — "N not in BC" archive counters under-count a manual-priced non-BC part (volatile `bcVerify.status` recompute vs the durable `bcNo` the B058 circle now uses); pairs with auto-pricing re-enable. Full detail: INBOX.md triage log + STATUS.md "Next Session".

### 🐛 Bugs (B###)
- **B051 — "Triangle (& multi-point line) markup doesn't render on the drawing overlay"** [✅✅ SHIPPED PROD v1.24.13 (2026-07-22, release `c061a06b`) · HIGH · S — ⏳ Jon prod-verify (couldn't test on test: TEST ribbon covers the markup tools → [[G015]]; verify on prod: draw triangle/line + judge circle→oval)] — FIX: unit `viewBox`+`preserveAspectRatio="none"` on the overlay svg (:31025) + child coords `%`→0-100 unitless + `vector-effect="non-scaling-stroke"` (pins stroke/dash to px). Triangle+multi-line now paint; rect/circle/line position-identical, strokes identical. ⚠️ **KNOWN TRADEOFF for Jon's eyeball:** under `preserveAspectRatio="none"`, `<circle>` stretches to an **ellipse on non-square (portrait/landscape) drawings** (still encircles the same region). Cosmetic; if round circles matter, follow-up = `<ellipse>` w/ aspect-compensated rx/ry. Storage/MARKUP-list untouched. — Engineer feedback (triage `docs/ENGINEER-FEEDBACK-TRIAGE.md`). Root cause CONFIRMED: the Drawing-Review overlay SVG (`:31025`) has no `viewBox`; the triangle `<polygon points="…%">` (`:31032`) + multi-point `<polyline points="…%">` (`:31028`) use PERCENTAGES in `points`, which SVG disallows → never paint (record IS created, shows in MARKUP list). Fix: `viewBox="0 0 100 100" preserveAspectRatio="none"` + drop `%` on children. **Standalone quick win — land FIRST (F034/F035/F037/F038 build on the fixed overlay).** *(source: engineer feedback, triaged 2026-07-22.)*
- **B050 — "Post-review assignments generate no bell notification"** [OPEN · MED · 🔨 building] — Coach found (via [[B049]] scope): only PRE-review assignment writes a bell notif; POST-review writes none, so a post-reviewer gets no ping. Jon (2026-07-22): add it. Building (worktree, on top of B049): create a `post_review` bell notif at each post-review assignment site (mirrors the `pre_review` create at ~:35628/:35751, targets the assignee uid) + add `post_review` to B049's navigable gate + `handleNotifClick` branch (opens project). Client-side add-only, no CF/schema change, all-users. **✅✅ SHIPPED PROD v1.24.9** (2026-07-22, release `81e47ebb`) — single create at the PO-received handler (`PoReceivedModal.onDone`, ~:40424) targeting `postReviewAssignedTo || bcDesignerUid`; `post_review` added to B049's gate + handler. **Nuance:** post-review has no manual assign-picker — it's auto-assigned to the project's designer at PO-receipt, so the bell fires then (matches the post-PO production-review workflow). Lands on next test build; bundles to prod with the batch. *(source: Jon 2026-07-22.)*
- **B049 — "Review notifications don't navigate to the project needing review"** [✅✅ SHIPPED PROD v1.24.9 (2026-07-22, release `81e47ebb`) · MED] — Jon relaying Andrew (2026-07-22): clicking a review-assignment notification does NOT take him to the project. **Coach root cause** (plan: `docs/B049-F031-2-NOTIF-NAV-COACH.md`): the bell rows gate `cursor`+`onClick` on `supplier_quote` only (predates F031 #3, NOT a regression) AND `handleNotifClick` has no `pre_review` branch — so `pre_review` (+ already-written `customer_review`/`issue_report` handlers) are all dead. `projectId` IS on the payload → pure gate+branch bug. **Building (worktree):** widen gate to a type→target test (keeps supplier_quote `_rfqHandled` suppression) + add `pre_review`/`unlock_*` branch (opens the project via `handleOpen`, same as the email `?openProject=` path) + re-enables customer_review/issue_report deep-links. Hosting-only. ⚠️ **Coach flag → Jon Q:** post-review assignment writes NO bell notification at all (only `pre_review` does) — if Andrew's failing notifs are post-review expectations, widening the gate can't help (nothing to click); confirm pre vs post + whether to add a `post_review` bell notif (separate additive item). Verify fix on a reviewer account. *(source: Jon relaying Andrew 2026-07-22.)*
- **B048 — "To-Do rail Sales Pipeline data lags a few seconds on page reload"** [OPEN · MED · 🔨 building (A+B)] — Jon (2026-07-22): on reload the right-side To-Do pane takes a few seconds to populate its Sales Pipeline pills. **Coach root cause:** not a slow query — the pills read `window._arcSalespersonCache` (loaded late via background BC auth ~:48825) and its arrival fires NO re-render → pills show a createdBy-only partial count, corrected only by the fragile 3s `syncBcProjects` timer (:49004, bails on same-ref); + no loading state → visible "jump." **Jon chose fix A+B:** (A) reactive recompute via a shared `salesCacheVer` counter bumped when the roster lands (rail AND board memos depend on it — `_isMyProject` predicate NOT forked); (B) localStorage warm-cache hydrated synchronously at module load → correct pills on first paint, live fetch reconciles. Roster-only persist (BC email↔code, data-safe), success-only, BC-offline fallback intact. ✅ SHIPPED-TO-TEST V.027 (A+B, master `5b666d4d`). Build: module-load localStorage hydrate (B) + `_landSalespersonCache()` (persist-on-success-only, roster-only) at 3 fetch sites + app-level `salesCacheVer` counter threaded into TodoRail `_sections` memo (A); board scope is an unmemoized IIFE so it recomputes on the same App re-render → rail+board recompute together, `_isMyProject` NOT forked. **✅✅ SHIPPED PROD v1.24.9** (2026-07-22, release `81e47ebb`). Coach review: ✅ APPROVE (SSOT verified — no `React.memo` in file, board scope is a live IIFE → rail+board recompute together on the version bump, `_isMyProject` not forked; warm-cache roster-only+persist-on-success-guarded; BC-offline degrades; no render loop). Prod-ready. One non-blocking pre-existing nit: in-memory `_arcSalespersonCache=d.value||[]` assigns before the length guard, so a rare empty-but-successful BC response briefly wipes the in-memory roster (localStorage protected; self-heals next fetch) — predates B048. Bundles to prod with divider/F031#4/B049/B050 once F031#2 fix lands + Jon re-verifies. *(source: Jon 2026-07-22.)*
- **B047 — "ProjectTile owner/EDITING name overflows the tile at narrow width"** [✅✅ SHIPPED PROD v1.24.8 (2026-07-22, release `fa3a8110`) · MED] — Jon (2026-07-22, screenshot at min tile width): Row-1 right-side owner name / "<FIRST> EDITING" ran OFF the tile. FIXED (worktree `adaf2356` → master `5de11e66`): Row 1 `overflow:hidden`; owner name shrinkable+ellipsis, right-justified; EDITING indicator `whiteSpace:normal`+`maxWidth:64` so "RYAN EDITING" wraps to 2 lines. Presentation-only. On matrix-arc-test V.025. *(source: Jon 2026-07-22.)*
- **B046 — "To-Do pane header shows 'To-Do List' (no first name) for some users (Andrew)"** [✅✅ SHIPPED PROD v1.24.5 (2026-07-22, release `d07eb7e9`) — userFirstName + memberMap now fall back to auth displayName→email-prefix when profile firstName unset; ships to prod next batch] — the F025 pane header `` userFirstName?`${userFirstName}'s To-Do List`:"To-Do List" `` (~:45200) falls to the bare label for Andrew → `userFirstName` is empty for him (works for Jon). Coach tracing the `userFirstName` source (member-doc `firstName`? auth displayName? owner-only path?) + why it's empty for a team member. Fix likely a fallback (displayName / email-prefix / member firstName) and/or populate firstName for all members. *(source: Jon relaying Andrew 2026-07-22.)*
- **B045 — "In-app notification BELL has never fired/shown — for any trigger, since it was built"** [✅✅ SHIPPED PROD v1.24.4 (2026-07-22, release `c2a78bf1`) — index-free listener fix; verified live by Jon; now live for ALL users. Follow-ups → F031 (per-item clear + RFQ deep-link + handled-note).] — Jon (2026-07-22): has NEVER seen a bell notification for anything (supplier-quote submissions, engineer questions, etc.) since the feature shipped; historically low-priority so unmentioned. **★ Escalated to HIGH because F029's awareness/notify vision DELIVERS via this bell — Phase B can't notify until the bell works.** Notifications stored `users/{uid}/notifications/{id}`; listener ~:48045 (`where read==false`+`orderBy createdAt desc`); badge amber+red on unread. "Nothing ever, for anything" → likely a single systemic break, not per-trigger. Prime suspects (Coach diagnosing): missing composite INDEX for the `read==false`+`orderBy createdAt` query (silent listener error → 0 results); rules reject CREATES (a tightened create rule requires `from` @~:38562 — a create path omitting it fails silently) or READS; uid mismatch (written to wrong user); or listener never mounted. **★ ROOT CAUSE CONFIRMED (Coach, `docs/`-less inline diag):** the bell listener (`src/app.jsx:48046-52`) query `.where('read','==',false).orderBy('createdAt','desc').limit(50)` **requires a composite index `(read ASC, createdAt DESC)` on `notifications` that was NEVER created** (`firestore.indexes.json` has only `projects_archive`), AND the `onSnapshot` error callback is **empty `()=>{}`** → the query silently errors on every mount, `notifications` stays `[]` forever, badge never lights. **Other 3 links HEALTHY** (kills alternatives): CREATE works — CF `onSupplierQuoteSubmitted` writes via Admin SDK to `users/{after.uid}/notifications` (`functions/index.js:645-656`), client creates stamp `from` (:35575/:38563/:38595); RULES allow owner reads (`firestore.rules:12-13`) + the `from`-required create (:24-26); render is fine but starved (`:49122-31`). It's INDEX/read-side. **FIX (low-risk):** (a) add the composite index to `firestore.indexes.json` + `firebase deploy --only firestore:indexes` (zero code change; once built, historical unread surface), OR (b) index-free: drop `.where`, `.orderBy(createdAt desc).limit(50)` + client-filter `read===false` (ships with hosting, instant, no index-build); **regardless, replace the empty error handler with a logging one** (this anti-pattern hid the bug for the feature's whole life). **★ F029 PREREQUISITE — Phase B can't notify until this is fixed** (reuse this same collection/listener, don't add a parallel path). *(source: Jon 2026-07-22; Coach diagnostic.)*
...
- **B018 — "Send-block overlay counts only pricing-incomplete rows, not all red rows (confusing)"** [✅✅ SHIPPED PROD v1.24.12 (2026-07-22, release `c17ebbab`) · MED — ✅ CONFIRMED LIVE ON PROD: item 3044102 green (Problem B phantom-red fix) + split-by-reason banner working ("8 block Send: pricing · 16 flagged for lead time: will RFQ, won't block", Problem A). Jon verified item 3044102 green on test; Coach money-path APPROVE. Shipped B018-ONLY via F030-revert isolation (F030 v1 held off prod: revert e24be048 → deploy → reapply 9fbfc5d7).] — **Coach confirmed the null-`bcPoDate` edge is CORRECT (do NOT weaken to `bcPoDate||priceDate` — every user-priced-as-BC path stamps bcPoDate; a null-bcPoDate BC row is genuinely "no PO"/not firm → correctly red). Nits N1 (temp #192 instrumentation drift, diagnostic-only) + N2 (other advisory surfaces could fold onto the accessor later) — non-blocking. Coach ROOT CAUSE: NOT a type-parse defect — a dual-FIELD divergence.** `_isBomRowFlaggedRed` (~:16298/16300) + `findIncompleteQuoteItems` (~:16422/16423) read `r.priceDate`, but the Priced-Date COLUMN (~:30592/30611) reads `bcPoDate` for BC rows. BC refresh paths (5-min poll ~:25185-25194, backfills ~:25137/28237) advance `bcPoDate` WITHOUT syncing `priceDate` → BC row shows a fresh date but reds; +1¢ clears it (edit stamps `priceDate:Date.now()`). **Blast radius = money-path:** the `priceDate`-only read also (1) over-counts send-block → **disables Send** on complete data, (2) trips `_hasRedRows` (~:1703) → **silently downgrades quote to Budgetary**. **FIX (building):** ONE `_effectivePriceDate(r)` accessor (BC→`bcPoDate` else `priceDate`) routed through red-flag + send-count + column (SSOT, read-only, Data-Retention-safe; no write/field change). **+ Problem A (Jon chose split-by-reason overlay):** banner "N block Send: pricing · M flagged for lead time: will RFQ, won't block" (M = LT-only reds). Verify post-fix: PRJ402100 `3044102` → green + Send-enabled + not auto-Budgetary. — the send-block cover says "N items incomplete (incomplete pricing)" counting ONLY pricing-incomplete rows (e.g. 2: 8660023 + 8617502 — price present but `priceDate` NULL, the #178/#179 pattern), while the BOM shows more red rows (6), because missing/stale LEAD TIME also reds a row but does NOT block send (it's RFQ'd). Users can't reconcile "6 red but 2 counted." The pricing-blocks-vs-lead-time-RFQs split is LIKELY intentional (confirm it's not #178/#179-class predicate drift). FIX (Jon's call): probably CLARIFY — overlay lists/explains ALL red rows by reason ("2 block send: pricing; N flagged for lead time: will RFQ, don't block") rather than block-on-any-red (which would over-block the RFQ workflow). Also open: WHY do 4 non-blocking rows (9345610/8660021/8660022 dated Jun-29 ~10d, 8617500 Jul-8) render red when priceDate isn't stale (<60d)? → investigate `_isBomRowFlaggedRed` vs the send-block predicate (dual-consumer alignment, #175/#178/#179 class). *(source: Jon 2026-07-08 PRJ402096.)* **★ DECISIVE REPRO folded in (Jon, PRJ402100, 2026-07-10):** item `3044102` rendered RED with ALL reqs met (Priced Date + Lead Time + BC satisfied); bumping the price **+1 cent CLEARED the red** → points squarely at `_isBomRowFlaggedRed`'s **priceDate read/staleness compare** (a valid priceDate mis-evaluated as stale; the 1-cent edit refreshes priceDate/state and clears the flag). Audit `_isBomRowFlaggedRed` + the priceDate staleness comparison. (Distinct project/cleaner repro than the PRJ402096 observation; same likely root — kept under B018.)
...
- **B032 — "save-on-open wipes flat/panels-less projects"** [✅ RESOLVED — ff1ea466 / v1.23.12, verified present on prod v1.24.36 (see resolution note at end); the "never folded to master" claim was WRONG] — opening a flat/panels-less project fires the eager save-on-open effect (`app.jsx` master ~:37814/:37863) which persists `migrateProject(stale dashboard-cached init)` = `{panel-1, bom:[], status:'draft'}` WHOLESALE → wipes the BOM + flips status to draft. The B016-2 merge is bypassed (no matching server panel → `cp` undefined) and the nBom belt is skipped (first-save, unset high-water). **LIVE ON PROD v1.23.11** — the C137 Phase A fix (delete the save-on-open effect) was Jon-approved but landed on the shelved `claude/phase-b-bom-merge`, never folded to master. Reproduced on the b016-23-merge test build (PRJ402063, throwaway) 2026-07-12. **FIX:** delete the save-on-open effect + dead `didMigrate` ref (C137 Phase A) → **emergency prod deploy** (independent of the B016 matrix); + defense-in-depth (panel-level union when the merge can't run, nBom belt in `saveProjectPanel`, first-save high-water seed). *(source: Coach C148 diagnosis, 2026-07-12; Freddy.)* **✅ RESOLVED 2026-07-24 (Coach re-diagnosis + Freddy git-verify):** the C137 Phase A fix (delete the eager save-on-open effect + dead `didMigrate` ref) DID land on master — commit `ff1ea466`, shipped **v1.23.12** (2026-07-12, same day, AFTER the "LIVE ON PROD v1.23.11" note) — and is present on current prod **v1.24.36**. Git-verified: `ff1ea466` is an ancestor of master; the only `safeSave(uid,migrateProject(init))` remaining is inside the explanatory comment (`:39796`); `migrateProject(init)` now runs IN-MEMORY only (`setProject`/`projectRef` at `:38955`/`:39435`/`:39449`, no persist) → migrates on snapshot, persists on the next real edit — the wipe *write* is gone with zero data-loss. **The "never folded to master" was wrong; there was no active prod exposure post-v1.23.12.** Blast radius (historical): legacy flat/panels-less docs only (paneled projects were the separate M2/M3 / B016 track). Remaining = OPTIONAL defense-in-depth (panel-level union when `cp` undefined, nBom belt in `saveProjectPanel`, first-save high-water seed) → fold into the B016 concurrent-BOM-merge track as LOW/MED hardening, NOT an EMERGENCY.
...
- **B023 — "Quote Summary line-row overflow clips the total price (long status pill)"** [✅✅ SHIPPED PROD v1.24.4 (2026-07-22, release `c2a78bf1`) — status-pill wrapper made shrinkable so a long status truncates instead of clipping the price] — in QUOTE SUMMARY a panel-summary row's status text (e.g. "IN PRE-REVIEW") is too long → pushes the total price out of the row box (price clipped at the right edge; Jon screenshot). Jon's suggested fix: truncate "IN PRE-REVIEW" → "IN ETR" (Engineering Technical Review); also add min-width/ellipsis so any long status can't clip the price. Pill wording ties to F003 Tech-Review. *(source: Jon 2026-07-10, via Intake.)* **★ FIXED (2026-07-22, worktree `811a6574`, pending merge+deploy):** robust layout fix — made the status-pill wrapper `flexShrink:1,minWidth:0,overflow:hidden` so a long status truncates instead of shoving the `flexShrink:0` price off-edge (price width always reserved, any long status). Presentation-only, one line @~:36904. Label abbreviation ("IN ETR") left as an optional Jon follow-up (would affect every `Badge` surface via the map @:16649). To merge onto master after F025 3a commits, re-validate in main tree (worktree used --no-verify), deploy with 3a.
...
### ✨ Features (F###)
- **F075 — "Manual Get-Prices button (Mouser/DigiKey) + Confirmed/Budgetary-gated apply + opt-in record-as-optional"** [✅✅ SHIPPED PROD v1.24.53 (2026-07-29)] — restored the 📥 Get Prices button (manual, API-only: Mouser/DigiKey; `API_PRICING_ENABLED=true`, scraper/AI kill-switches stay OFF). Phase 2 (this session) reworked the apply around Jon's model: Budgetary/empty/stale rows priced on-row **keeping existing supplier** (fixes findings #1/#3 — no DigiKey overwrite) + strip `bcPoDate` (Priced-Date fix, first shipped as the v1.24.52 RFQ-send hotfix, finding #2); Confirmed rows protected + offered in an **opt-in confirm modal** to record the API vendor's price as an OPTIONAL BC Purchase Price (F072 machinery; F071 BC-down + $0 guards; vendor# from `config/vendorConfig`; UoM via `bcGetItemBaseUom`). Stale-Confirmed = keep-protected (Jon). Coach APPROVE-WITH-NITS (UoM folded). Jon Test-verified #2/#3; #1 code-verified. Plan: `docs/F075-PHASE2-BUILD-PLAN.md`. ⏳ **Optional-BC-write half is DORMANT until F077 (vendor-# config) exists.**
- **F077 — "Vendor Sync config UI: map an API source (DigiKey/Mouser/RS-Online…) → its BC vendor number"** [OPEN · MED · PREREQUISITE for F075 optional-write · = the "New-API-source BC-vendor check" intake] — F075's optional-BC-write reads `digikeyVendorNo`/`mouserVendorNo` from `users/{uid}/config/vendorConfig`, but there's **no clean UI to set them** (only the mass "Update All DigiKey Prices" sync writes them as a side-effect; Mouser has no field; direct write classifier-blocked). Build a small config surface to set/auto-detect the vendor # per API source (+ verify the source's vendor exists in BC — the RS-Online use case Jon flagged). Verified BC #s in the UAT sandbox: DigiKey=`V00196`, Mouser=`V00304`. **▶ Jon to pick unblock path AM 2026-07-30 (3 options in STATUS.md).** *(source: Jon 2026-07-29.)*
- **F076 — "Supplier portal: 'Enter Pricing & Lead Time manually' button (no PDF)"** [OPEN · PARKED · S (~30-40 lines) · plan ready] — generalizes the existing `leadTimeOnly` portal path (skip-upload → editable review grid → same `handleSubmit`). Plan: `docs/F076-PORTAL-MANUAL-ENTRY-PLAN.md`. Held pending Jon go. *(source: Jon 2026-07-29.)*
- **F034 — "Click a markup in the review list → highlight its shape on the drawing"** [OPEN · MED-HIGH · M] — Drawing-Review MARKUP-list onClick (`:31233`) + notes-list (`:31150`) only jump page, no highlight. Add `selectedShapeId`/`selectedNoteId` → pulse/flash the matching overlay element (`:31026`/`:30960`). Part of the markup-interactive mini-epic (with F035/F036); builds on B051's overlay fix. *(engineer feedback, triaged 2026-07-22.)*
- **F040 — "Shape notes render beside their shape, movable, with a leader line to the shape"** [OPEN · MED-HIGH · M — ✅ SCOPE READY `docs/F035-F040-INTERACTIVE-MARKUP-SCOPE.md`; build w/ F035-move (one PR)] — add-only `noteX/noteY` on the shape (note already lives on `shape.note`, no separate entity/link); label = movable HTML div (mirrors the note-drag machinery :31033); leader = SVG line to derived centroid, live-tracked; legacy notes default to centroid+offset (no migration). — Jon (2026-07-22, testing B051 on prod): a shape's note should render as a label positioned NEXT TO the shape, be independently MOVABLE (drag the note), and draw a **leader line** connecting the note back to its shape. Needs a data model (add-only per Data Retention): note position/offset + shape linkage + leader anchor; existing shape-notes (`shape.note` text, ~:31073/:31245) default to beside-shape with no migration. Pairs with F035 (same overlay/interaction model). *(source: Jon 2026-07-22.)*
- **F035 — "Move + resize a placed markup shape"** [OPEN · MED-HIGH — ✅ SCOPE READY `docs/F035-F040-INTERACTIVE-MARKUP-SCOPE.md` · build split: **F035-move (S-M, ship first — Jon's stated need)** → F040 → F035-resize (L, fast-follow)] — MOVE = a new Select/Move `↖` tool + per-shape `pointerEvents:"stroke"` gated on select-mode + translate coords by bbox-clamped delta, persist via the note save path (no new fields). Resize (per-type handles) deferred. 6 open decisions (all Coach-rec'd) for build kickoff. *(Jon prioritized 2026-07-22.)* shapes render in a `pointerEvents:none` SVG (`:31025`) — only NOTES drag today (`:30966`). Add drag + per-type resize handles (rect corners / circle radius / triangle verts / line ends). Highest user value of the cluster, biggest build. Rides merge-guarded `reviewShapes` (no parallel storage). Builds on B051. *(engineer feedback, triaged 2026-07-22.)*
- **F036 — "Edit a markup's note text after placing (shape-notes) + wrap-not-scroll"** [OPEN · MED · S-M · ⚠ VERIFY-FIRST + Jon confirm] — note-PIN text is already click-to-edit (`:30988`) — maybe shipped 2026-07-14 → possible no-op for pins. Genuinely missing: SHAPE-notes are one-shot at creation (`:31073`), MARKUP list only displays `s.note` (`:31245`). Switch the single-line note `<input>`s (`:31021`/`:30986`) to `<textarea>` (wrap) — folds in the "wrap not scroll" ask. **Jon/engineer: which "note" — pin (maybe done) or shape-note (missing)?** *(engineer feedback, triaged 2026-07-22.)*
- **F037 — "Highlighter markup tool"** [OPEN · MED · M] — new entry in `markupTool` set (`:24914`) + toolbar (`:30934`) + semi-transparent thick stroke in the overlay. Straightforward once B051's viewBox lands. *(engineer feedback, triaged 2026-07-22.)*
- **F038 — "Free-text markup (plain text labels on the drawing)"** [OPEN · LOW-MED · M · ⚠ PRODUCT DECISION] — note pins already place text (# bubbles, INT/EXT); engineer wants plain labels (e.g. part crosses) without the chrome. **Jon call: distinct `type:"text"` shape vs a note-style variant?** Scope depends on answer. *(engineer feedback, triaged 2026-07-22.)*
- **F039 — "Escape reverts an in-progress part#/qty BOM-cell edit"** [OPEN · MED · S · BOM-edit, not markup] — generic BOM cell `<input>` (part#/desc/qty, `f`-loop `:30494-30539`) has `onBlur`/`onFocus` but no `onKeyDown` → Escape doesn't revert. The PRICE cell already does (`:30558`). Mirror it. NOT money-path (discards uncommitted edit only); NOT a B031 dup (B031 = price-CLEAR guard). *(engineer feedback, triaged 2026-07-22.)*
- **F033 — "Persist the To-Do rail on EVERY nav tab (not just the Sales board)"** [OPEN · scoping] — Jon (2026-07-22): the To-Do list should always be in view while working anywhere in ARC. Today the F025 rail renders only on the Sales board status view (`_railEligible=!forceView&&groupBy==="status"` ~:44870); it's absent on Purchasing/Engineering/Production/Items-Vendors. Make it a **persistent global right rail** across all nav tabs. Not a flag-flip — the pane lives inside the Sales `Dashboard`; Items/Vendors isn't even a Dashboard → likely lift the rail to an **app-level component** fed by app-level `projects`/`rfqCounts`/`memberMap`/`user`, with pill-clicks that `setNavTab('sales')` + carry a pending board-focus (board actions only apply on the Sales board). **★ Coach scoped → Option A (app-level `<TodoRail>`, effort M):** rail's data deps already app-level (projects/uid/userFirstName/_appCtx.permissions + module helpers; doesn't use rfqCounts/memberMap) → extraction + pending-focus plumbing + layout wrap, not new logic. **Jon decisions:** rail on ALL top-level tabs (Sales/Purchasing/Engineering/Production/Items); **SUPPRESS inside a project (ProjectView)** + **on the future F030 dashboard page** (suppression hook now, active when F030 lands — F030 IS the full list, rail would be redundant); collapse-state SHARED across tabs; right side, hidden while AI Assistant open. Pill-click → `setNavTab('projects')` + app-level `pendingFocus` the Sales Dashboard consumes. ✅✅ SHIPPED PROD v1.24.6 (2026-07-22, release `c1054057`) — Coach APPROVE WITH NITS (race verified, no Sales regression). ⏳ Jon prod-verify: rail on each tab · pill-click from non-Sales tab · collapse persists · no double right-pane w/ AI Assistant. **Follow-up UI (Jon 2026-07-22): ✅✅ SHIPPED PROD v1.24.9 — full-height divider** (release `81e47ebb`; was master `c6bb03b1`). Root cause: the open-rail `<aside>` (:44761) had `height:"100%"` which overrode flex-stretch and collapsed to tile-content height → border stopped short. Fix = `height:"100%"`→`alignSelf:"stretch"` (1 line); line now spans the full `100vh` row regardless of tile count. Collapsed strip already stretched (untouched). Presentation-only. Bundles to prod with B048. *(source: Jon 2026-07-22.)*
- **F032 — "Role-differentiated To-Do dashboard (Salesman / Reviewer / Designer views)"** [✅✅ SHIPPED PROD v1.24.5 (2026-07-22, release `d07eb7e9`) — `_dashboardRoles` renders stacked union of held roles; salesman pills gated on ACTUAL salesperson (fixes Andrew's phantom RFQs); NEW `permissions.designer` flag (Team-UI); Reviewer section (pre/post-review assigned-to-me); Designer section (bcDesignerUid + service type); admin gating on RAW flag (Jon+Coach — non-salesperson admin keeps Sales Pipeline). ⏳ Jon verify on test (+ set Andrew's Designer flag in Team settings). Ships next prod batch.] — the shipped F025 To-Do pane is SALESMAN-centric (8 sales pills, scoped `createdBy`/salesperson) → wrong for other roles. Andrew (Reviewer + Designer) feedback (via Jon 2026-07-22): (item 2) his pane wrongly shows "2 RFQs to address" — he doesn't handle RFQs; (item 3) as a **Reviewer** he should see only **IN PRE-REVIEW + NEEDS POST-REVIEW** (projects assigned to HIM); (item 4) as a **Designer** he should get **Engineering Design** tiles. → Make the To-Do dashboard **role-aware**: detect the user's role(s) (`permissions.reviewer` F003, a designer/engineer assignment, salesperson/createdBy; a user can be MULTIPLE — Andrew=Reviewer+Designer) and show role-appropriate buckets + the right project SCOPE per role (a reviewer's "my projects" = assigned-to-them-for-review, not createdBy). Applies to the F025 pane AND the future F030 dashboard page. **Coach scoping** the role/assignment model + role→buckets mapping + why Andrew sees RFQs. Open Qs: multi-role handling (union vs toggle), what "Engineering Design" buckets are, whether a designer-assignment field even exists. *(source: Jon relaying Andrew 2026-07-22.)*
- **F031 — "Notification UX: per-item Clear button + deep-link straight to the RFQ + already-handled note"** [OPEN · scoping] — Jon (2026-07-22): (1) clicking a notification navigates to the project and the message is lost → add a **✕/Clear button per notification** that marks it read + removes it from the list WITHOUT navigating and WITHOUT closing the dropdown (keep the list open); (2) supplier-quote notifications say "click to review" but only land on the **project**, not the RFQ → **deep-link straight to the specific RFQ submission** that needs receiving (open PortalSubmissionsModal focused on that `uploadToken`); (3) if the RFQ was **already received/handled** (`rfqUploads/{token}.status` imported/dismissed) → show a **note** in the notification saying so + that it can be cleared, instead of the dead "click to review." Likely needs an add-only enhancement to the `onSupplierQuoteSubmitted` create payload to carry the `uploadToken` for the deep-link. **★ Coach scoped (`rfqUploadId` already on the notification → no create-payload change for the deep-link).** Build order #1→#3→#2. **#1 (Clear ✕) ✅✅ SHIPPED PROD v1.24.7** (2026-07-22, release `94226afa`) — per-notification dismiss, keeps dropdown open. **#3 (handled-note) ✅✅ SHIPPED PROD v1.24.8** (2026-07-22, release `fa3a8110`) — app-level `rfqStatusMap` listener (`_listenRfqUploadsTeamScoped`, no statusFilter, keyed by token=`rfqUploadId`); handled (imported/dismissed) RFQs show "✓ Already received — safe to clear" (green) + drop the cursor/navigate; still-submitted unchanged. Coach light-review PASS (handled-set={imported,dismissed}; warmup defaults to safe review label). Awaiting Jon prod OK. **#2 (deep-link to the specific RFQ submission) ✅✅ SHIPPED PROD v1.24.9** (2026-07-22, release `81e47ebb`) — Jon re-verified V.029 + Coach re-review APPROVE (latch correct: opens on submissions-land, stays, no double/wrong-project open; 2 non-blocking nits — rare abnormal-data dead-click, cosmetic `_navigable` projectId asymmetry).** Jon V.028 live-test: click → nothing + notification REMOVED (no console error). **Root cause = TWO pre-existing bugs (both predate v1.24.8 — NOT F031#2 regressions; the deep-link was just the first tested path that surfaced them):** (1) CRITICAL — `handleNotifClick` marked read UNCONDITIONALLY before the `if(proj)` guards, so a `projects.find(projectId)`-miss (archived/filtered/teammate-owned/not-yet-loaded) silently no-op'd but still consumed the notification; (2) flicker — auto-open effect cleared its own App-level trigger in the same tick, racing the rfqUploads listener. **Fix (`9e0fc29a`):** mark-read only on a `navigated` success (else keep + graceful arcAlert explaining why); auto-open split into 2 effects + durable `autoOpenIntent` latch, `showPortalModal` sole latched visibility. Money-path untouched; B049/F031#4 intact. ⚠️ **Deeper follow-up candidate:** a notification whose project is archived/filtered/teammate-owned now fails gracefully but still can't OPEN — making those openable-from-notification is separate (needs the project loaded on demand). Plans: `docs/B049-F031-2-NOTIF-NAV-COACH.md`. **#4 (batch-clear safe-to-clear) ✅✅ SHIPPED PROD v1.24.9** (release `81e47ebb`) — "✓ Clear received (N)" dropdown-header button clears ALL safe-to-clear notifs in one pass; predicate factored to ONE `_isNotifHandled` helper (SSOT, used by row-note + button); hidden when count 0; keeps dropdown open; client-only (`read:true`). Jon: ship F031 to prod as each piece passes. *(source: Jon 2026-07-22.)*
- **F030 — "Dedicated User Dashboard PAGE (personal, separate from the all-projects board)"** [✅✅ SHIPPED PROD v1.24.13 (2026-07-22, release `c061a06b`) — Jon "layout looks good for now" after 3 layout rounds (r1 side-by-side/one-row-pills/Email-scaffold → r2 Review|Engineering side-by-side/shorter-tiles/skinnier-cols → r3 +50% cols/skinny far-right bell column). F030 v1 was held off prod during the B018-only ship (revert/reapply), then bundled here w/ B051 + F029 Email panel.] — **Jon layout feedback on V.033:** (1) NEEDS ATTENTION + MY PROJECTS side-by-side; (2) Sales Pipeline pills = one full-width row + widen the dashboard; (3) add an **Email notifications section ABOVE the ARC bell list, wider** for email subject + preview body — this is the **F029 Outlook mount point, built as a sized scaffold now** (email data wired when F029/Outlook connects, per the F030-before-Outlook sequencing). Layout lane building (worktree, presentation-only). — **Coach scope: build-ready, effort M, "the hard part is already wired"** (`docs/F030-USER-DASHBOARD-PAGE-SCOPE.md`): F033 rail-suppression hook already exists as a no-op waiting for `navTab==="user_dashboard"`; every data path is an existing SSOT → v1 is assembly. **Jon decisions:** (1) nav tab "MY DASHBOARD" FIRST/leftmost; (2) notifications window UNREAD-ONLY v1 (reuse bell's live onSnapshot, no new listener); (3) body = attention-style project rows + $ totals (reuse scoped set + buildArcContext total math, not the heavy kanban tiles). **Build = page-mode `TodoRail` (reuse pills/attention/roles/timers/RFQ — no fork) + ONE shared `renderNotifRow` (bell + page) + nav tab + F029 insertion placeholder.** ARC-native, standalone, zero Outlook dep. Was Coach scoping the build-ready plan: nav-tab wiring + page-body switch, the F033 rail-suppression hook for this page, REUSE strategy for the pills + Needs Attention list (page-mode TodoRail vs factored shared logic — no fork of `_dashboardRoles`/timers/`_rfqAwaitingSummary`), the **live real-time notifications window** (reuse the bell onSnapshot + `handleNotifClick`/`_navigable` + clear helpers; unread-only vs history), and which F025-v2 full-page sections (ECO list, Quotes-Sent follow-up, $-in-tiles) are in-scope-now vs deferred. **★ SEQUENCING (Jon 2026-07-22): F030 ships BEFORE the Outlook tie (F029)** — F030 v1 = fully ARC-native (pills + attention list + project tiles + real-time notifications window), standalone-shippable, ZERO Outlook dependency; F029 tasks/mail/meetings mount into it as a later additive layer (scope leaves a clean insertion point, no restructure). — a standalone page (its own nav destination) dedicated to just the signed-in user's world, NOT the all-projects board: (a) a listing of the user's projects as tiles **with $ totals in the tiles**; (b) the To-Do attention list of the user's most relevant/critical projects needing attention (the F025 pill/timer logic); (c) a combined **Outlook + ARC** tasks/to-do list (F029 Phase A); (d) emails/meetings awareness (F029 Phase B); (e) **a real-time notifications window** (live-updating, reuse the bell's onSnapshot `notifications` state — Jon 2026-07-22, #2). Essentially the convergence surface for F025 (attention) + F029 (Outlook tasks/mail/meetings) as a personal "home." **Develop later** — Jon flagged it as a direction to be aware of + work toward, not a now-build. Depends on / consumes F025 + F029 pieces. *(source: Jon 2026-07-22.)*
- **F029 — "In-ARC time-management + Outlook integration (two-way To-Do sync · meeting/email awareness+notify · email↔Project linking)"** [OPEN · planning] — build on ARC's EXISTING MSAL/Graph app (research: `docs/OUTLOOK-GRAPH-TODO-RESEARCH.md`). **Jon decisions (2026-07-22):** (1) task sync = **TWO-WAY** (ARC↔Outlook Microsoft To Do, conflict=last-writer-wins); (2) **personal/private** per-user; (3) **"important email" alert rule = high-importance OR pinned OR subject/body contains "RFQ"/"request for quote"**; (4) **email→Project linking = first-class** (Jon: "extremely useful" — RFQ emails already tie to projects via `rfq_history`, so inbound replies / project#-mentioning emails snap to the matching project); (5) notify **while working in ARC** (foreground poll + existing bell — no server infra; tab-closed background = later optional phase). Scopes to add (incremental user-consent on the existing app): `Tasks.ReadWrite` + `Calendars.Read` (Mail.* already live). **Phased:** A=two-way To-Do sync · B=awareness/notify (meetings + important-mail via bell) · C=email↔Project linking. **★ Build-ready phased plan: `docs/F029-PLAN.md`** — A (two-way To-Do sync, `Tasks.ReadWrite`) → B (meeting + important-mail notify via existing bell, `Calendars.Read`) → C (email↔Project linking: shared `linkedEmails` subcollection + RFQ-reply auto-match via capturing `conversationId` on send) → D later (tab-closed background = server infra, out of scope now). Smallest slice A1 = ARC-local My-Tasks tab + consent probe + one-way push. **★ KEY FINDING: Outlook "pinned" is NOT exposed in Graph v1.0.** **Jon (2026-07-22): pinned is NECESSARY — do NOT substitute `flagged`; TABLE it for deeper research** (can pinned be reached at all — the fragile extended-property `$expand`, or another signal — research later). So F029 Phase B's important-mail rule = high-importance + RFQ-text for now, with **pinned as a research follow-up** before B ships (A + C unaffected). **Jon: START F029 after the current test batch (V.016) ships to prod** (not now). Other baked decisions (Jon-approved, veto-able): My-Tasks own rail tab · meeting lead 10min · linkedEmails subcollection · LWW · capture conversationId on RFQ send · confirm tenant user-consent. **★ FIRST SLICE 🔍 planning (Jon chose to pre-plan 2026-07-22, parallel to F030 layout):** wire the **F030 "📧 Email" scaffold** (subject + `bodyPreview`, click→`webLink`) with the user's relevant emails via the EXISTING Graph `graphSearchEmails`/MSAL — criteria high-importance OR subject/body contains "RFQ"/"request for quote" (pinned still tabled); personal/private, client-side (no email content to Firestore); poll/on-load refresh (not push) + graceful "Connect Outlook" state. This is the concrete first Outlook slice that drops into F030's mount point once F030 ships. **✅ SLICE-1 PLAN READY: `docs/F029-SLICE1-EMAIL-PANEL-PLAN.md`** — effort S–M, single file, reuses live Graph/MSAL (`ensureMsal`/`tryGraphTokenSilent`/`acquireGraphToken`/`graphSearchEmails` pattern), **zero new consent (Mail.Read already granted), zero Firestore** (email content ephemeral React state only, poll/on-load not push). New helper `graphFetchRelevantEmails` (2 calls merged: importance-filter + `$search` "RFQ"); replaces the F030 Email scaffold placeholder rows. **🔨 SLICE-1 BUILDING (Jon "get emails showing" 2026-07-22, worktree)** — decisions locked to Coach defaults: high-importance OR "RFQ" in subject/body · 30 days · `$top` 25 · manual ↻ + 5-min visible-tab poll (NOT real-time push — Graph has no free browser push) · High/RFQ tags + unread bold · graceful "Connect Outlook" when not signed into MS in-tab. Reads own mailbox client-side, ZERO Firestore persistence. **✅✅ SHIPPED PROD v1.24.13 (2026-07-22, release `c061a06b`) — Jon "emails showing, good for now"; Coach privacy APPROVE. · ✅ Coach privacy/auth review APPROVE WITH NITS — zero-persistence PROVEN (12 refs all ephemeral state, never in a save path / buildArcContext), no PII in logs (only status + Graph error JSON), own-mailbox only, no new scope, no surprise popup, clean interval. Nits (non-blocking): button handlers lack the unmount-guard the poll has; dead `conversationId` field. Cleared for prod on Jon's eyeball. ⏳ Jon eyeball the rendered panel (+ optional live Connect pass).** Helper `graphFetchRelevantEmails` (~:8662), state/effect (~:48445/48522), F030 Email render (~:49946). *(source: Jon 2026-07-22; plan lane.)*
...
### 🔧 General (G###)
- **G015 — "TEST env ribbon overlaps the drawing-review markup toolbar (blocks testing markup on test)"** [OPEN · MED · S · test-env only] — Jon (2026-07-22): the orange `🧪 TEST ENVIRONMENT` ribbon covers the drawing-review markup tools, so B051 (and any drawing-review feature) can't be verified on matrix-arc-test — forced B051's verification onto prod. Fix: adjust the test ribbon's z-index/position (or make it dismissible/auto-hide when a modal like Drawing Review is open) so it doesn't obscure in-modal toolbars. Test-env display only (prod has no ribbon); no prod-code risk. *(source: Jon 2026-07-22.)*
- **G014 — "Vertical spacing between per-page markup groups in the review MARKUP list"** [OPEN · LOW · S · cosmetic] — Engineer preference (triage `docs/ENGINEER-FEEDBACK-TRIAGE.md`). The MARKUP list (`:31232`) is currently FLAT (flatMap, no `Page N` headers); the requested spacing between page groups implies FIRST adding per-page group structure, then spacing — slightly more than a margin bump. *(engineer feedback, triaged 2026-07-22.)*
