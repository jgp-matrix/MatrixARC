...
### 🐛 Bugs (B###)
- **B049 — "Review notifications don't navigate to the project needing review"** [OPEN · MED · 🔍 scoping w/ F031 #2] — Jon relaying Andrew (2026-07-22): clicking a review-assignment notification does NOT take him to the project. Pre-existing gap (NOT an F031 #3 regression): the bell rows gate `cursor`+`onClick` on `n.type==='supplier_quote'` only, so review-type notifications have never been clickable/navigable. Pain (Jon): "once you click the notification you lose the message and forget what it was for" — the navigate must land somewhere useful. Scoping alongside F031 #2 (same subsystem = notification click navigation): Coach mapping the full notification-type→click-target inventory, whether review-assignment notifications are even created + carry a projectId, and the nav/deep-link primitives. Verify fix for ALL roles. *(source: Jon relaying Andrew 2026-07-22.)*
- **B048 — "To-Do rail Sales Pipeline data lags a few seconds on page reload"** [OPEN · MED · 🔨 building (A+B)] — Jon (2026-07-22): on reload the right-side To-Do pane takes a few seconds to populate its Sales Pipeline pills. **Coach root cause:** not a slow query — the pills read `window._arcSalespersonCache` (loaded late via background BC auth ~:48825) and its arrival fires NO re-render → pills show a createdBy-only partial count, corrected only by the fragile 3s `syncBcProjects` timer (:49004, bails on same-ref); + no loading state → visible "jump." **Jon chose fix A+B:** (A) reactive recompute via a shared `salesCacheVer` counter bumped when the roster lands (rail AND board memos depend on it — `_isMyProject` predicate NOT forked); (B) localStorage warm-cache hydrated synchronously at module load → correct pills on first paint, live fetch reconciles. Roster-only persist (BC email↔code, data-safe), success-only, BC-offline fallback intact. Building in worktree → Coach review (SSOT/boot-path) before prod. *(source: Jon 2026-07-22.)*
