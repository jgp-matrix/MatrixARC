...
> - **[BUG · LOW · Jon 2026-07-30]** BC Fuzzy Matches suggestion box shows the internal **MTX# instead of the Vendor Part#** — should display the vendor part number so the user recognizes the item. Display-only, low priority.
> - **[BUG · MED]** A few parts that ARE in BC (Item Browser finds them) are NOT matched by `bcFuzzyLookup` on extraction (debug log: "scanned 0 prefix candidates" for e.g. `800F-34RE100`). The auto-fuzzy-matcher misses some in-BC parts the Item Browser search finds — reconcile the two query paths. Surfaced during the BC pricing re-enable (v1.24.57).
> - **[BUG · MED]** **IP66 / 1200 wrong Part#** — on a dense spec-laden row the extraction model puts a rating (`IP66`) or a dimension (`1200`) into partNumber INSTEAD of the real Part# column value (`640014405` / `8660025`). These are REAL line items with a bad Part#, NOT spurious extra rows (do NOT delete/filter — a filter approach was tried and rejected). Fix must recover the correct Part# (model reads Part# column, or recovery if captured in additionalPartNumbers). Prompt reinforcement shipped v1.24.55 (nudge only, non-deterministic).
> - **[BUG · MED]** `?PN` / `~PN` part-number validity badges (from `verifyPartNumbers` → `panel.bomVerification`) **stopped populating** — the validity signal went dark this session (possibly API-529 / Firestore write-exhaustion starving the verify call). Investigate why bomVerification is empty.
> - **[Note]** F077 Vendor Sync UI (map `digikeyVendorNo=V00196`/`mouserVendorNo=V00304` into `config/vendorConfig`) is the PREREQUISITE for API→BC price writeback (built on Test V.074 this morning, not merged). Ship it to unblock the F075/pricing writeback loop.
