...
> **★ ACTIVE — QUICK-WIN BATCH (Jon approved 2026-07-07, via Intake).** Bundle as ONE patch → ONE Coach diff-review → ONE deploy. Members: **G006** (portal loading copy), **#190** ("Save Defaults"→"Save" + confirm full-modal commit), **#195** (strip false auto-BUDGETARY checklist entry on Print-as-Firm override), **B002** (state-aware approved-TR block message), **B001** (strip trailing dot from OAuth redirect origin), **B003** (hide the RFQ-Apply-modal bottom not-quoted list). **Sequencing (Jon 2026-07-07): batch FIRST, then G005 Phase 1** (both edit app.jsx → sequential, no parallel-collision). B011 + B005 EXCLUDED (money-path/TR-logic — need care, not a blind sweep). Building: Marc → Coach review → deploy.
...
- **B003 — "Supplier RFQ 'Apply' modal shows a noise not-quoted list"** [Backlog · in QUICK-WIN BATCH] — **SCOPE CLARIFIED (Jon, 2026-07-07): NOT the "Review Supplier Quote" modal.** It's the **Supplier RFQ "Apply" modal** — there's a box at the BOTTOM listing parts that were NOT quoted (because they belong to OTHER suppliers). It's just noise; no need to display it. **Fix = remove/hide that bottom not-quoted list.** *(source: Jon 2026-07-02, rescoped via Intake 2026-07-07.)*
...
- **G005 — "matrix-arc-test shares PROD Firestore (not data-isolated)"** [Backlog · infra · ⚠ ESCALATED 2026-07-07] — the test hosting target is in the same Firebase project as prod → same Firestore + Auth, so data-mutation tests there write the SAME DB as production. **The "safe now, all data is pre-launch/test" premise (2026-07-02) is BROKEN: real customer projects already exist in the DB** — proven 2026-07-07 when a B009 repro injected synthetic supplier-crosses onto 11 BOM rows of **PRJ402096, a REAL CUSTOMER project** (assumed sandbox; fully restored via the app's own save handler, no data lost — see [[feedback_confirm_throwaway_before_test_injection]]). So the shared-Firestore hazard is **LIVE now, not a before-launch concern**: any "test" mutation can hit real customer data. **Blast radius reaches the REAL ERP too:** the same 2026-07-07 injection pushed synthetic purchase prices into **Business Central** for 2 real items (B98110019 $42, 1032264 CS $99.99) via the apply→BC-price-writeback path — so "test" writes propagate beyond Firestore into the accounting/purchasing system of record (Jon deleting those directly). Mitigations: (a) real test-env isolation (separate Firebase project/DB) — the proper fix; (b) interim discipline — test-data injection only on a Jon-confirmed disposable project, mutate via app save handlers (guards), never raw writes. *(source: Marc #199 live pass 2026-07-02; escalated by the PRJ402096 near-miss 2026-07-07, via Freddy.)*
  ...
  - **STATUS: Plan APPROVED. Holding for Jon's explicit go-to-build** (per per-phase gating) → then Marc builds (client gates first, then Functions) → verify (§10, incl. near-miss replay on test with prod BC watched) → Coach review → Jon deploy checkpoint.
