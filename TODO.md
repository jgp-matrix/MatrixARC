...
- **F011 — "CSV BOM export: add Supplier, Lead Time, Priced Date columns"** [Backlog · quick] — extend `exportCSV()` (`src/app.jsx:27831`; current cols Qty / Part # / Description / Manufacturer / Unit $ / Cost Source / Notes) with three additional columns pulling **existing** row fields: **Supplier** (the row's vendor/supplier — mirror the on-screen value), **Lead Time** (`leadTimeDays`), **Priced Date** (`priceDate`). Additive, LOW-stakes report-field change; no data-model change. *(source: Jon 2026-07-09, via Freddy.)*
