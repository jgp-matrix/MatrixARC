...
- **F015 — "Same-user 2nd-tab: block/redirect instead of View-Only"** [Backlog · LOW/UX] — when the same user opens a project already open in another of their tabs (the L8 state-ii "open in another tab" modal), don't leave them in a confusing VIEW-ONLY duplicate. Jon's intent: force the 2nd tab to close + revert to the active tab. **Browser limit:** a page can't reliably close a tab it didn't open (`window.close()` only works on script-opened tabs) nor focus another tab → true force-close isn't reliable. Feasible forms: (a) **redirect the 2nd tab to the dashboard** with a note ("open in your other tab") so it's not a read-only clone; or (b) a hard blocking screen + one-click "Close this tab" (best-effort `window.close()`, else instruct); (c) same-browser BroadcastChannel nudge (focus not forceable). Recommend (a) or (b). Ties to ruling #8 (per-session tab keying) + gap #5 (tab-lease lifecycle). *(source: Jon 2026-07-10, via Freddy — from L8 re-test.)*
