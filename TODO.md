...
> **↳ SUPERSEDED → PIVOTED TO BC-WRITE (2026-07-28):** Jon's Test-V.063 verification reframed F072 — supplier must be a BC vendor (dropdown + Create-New), and Save must WRITE Purchase Price + Item Vendor Catalog to BC (not ARC-side). Re-scoped (Coach) + **rebuilt** (Marc, +258/-133, `config/itemAlternates` DROPPED — BC is single source of truth) → Coach money-path review **APPROVE-quality + M1 folded** (reject $0 price → no junk $0 PP to BC; all 3 write paths F071 HARD-BLOCK when BC down; no silent success) → committed `6a867470`, **Test V.064** (base v1.24.43). Read-only self-verified (clean load, itemAlternates gone, write helpers wired). **HELD at prod — writes hit the BC sandbox, so Jon runs the write-verify w/ a Network trace** (see STATUS "FOR JON"). Decisions LOCKED (Jon 2026-07-28): full BC-write · select-secondary also re-writes · hard-block when BC down · fetch base UoM · drop itemAlternates · re-write always. `docs/F072-SECONDARY-VENDOR-PLAN.md` (BC-write re-scope on master).
