...
- **B018 — "Send-block overlay counts only pricing-incomplete rows, not all red rows (confusing)"** [OPEN · MED — product decision] — the send-block cover says "N items incomplete (incomplete pricing)" counting ONLY pricing-incomplete rows (e.g. 2: 8660023 + 8617502 — price present but `priceDate` NULL, the #178/#179 pattern), while the BOM shows more red rows (6), because missing/stale LEAD TIME also reds a row but does NOT block send (it's RFQ'd). Users can't reconcile "6 red but 2 counted." The pricing-blocks-vs-lead-time-RFQs split is LIKELY intentional (confirm it's not #178/#179-class predicate drift). FIX (Jon's call): probably CLARIFY — overlay lists/explains ALL red rows by reason ("2 block send: pricing; N flagged for lead time: will RFQ, don't block") rather than block-on-any-red (which would over-block the RFQ workflow). Also open: WHY do 4 non-blocking rows (9345610/8660021/8660022 dated Jun-29 ~10d, 8617500 Jul-8) render red when priceDate isn't stale (<60d)? → investigate `_isBomRowFlaggedRed` vs the send-block predicate (dual-consumer alignment, #175/#178/#179 class). *(source: Jon 2026-07-08 PRJ402096.)*
- **B021 — "Extraction pricing phase hangs at 95% when an AI-fallback / BC pricing call stalls (no timeout)"** [OPEN · MED] — extraction freezes indefinitely at 95% "…pricing" when the pricing pipeline doesn't return. ROOT (Marc): `bgDone` (`app.jsx:15348`) sets 100% + auto-dismisses the banner in 4s, so a PERSISTENT 95% means bgDone never fired → a pricing phase didn't return. When BC returns few matches (`0 of 1/4`), the AI-fallback phases fire for the unpriced rows — Phase 3 `estimatePrices` + Phase 4 `aiEstimateLeadTimes` (Anthropic) + Phase 5 BC vendor-backfill (`~app.jsx:15135`); a stalled/never-resolving call there freezes at 95% (no timeout/abort). PRE-EXISTING, NOT P1/lease (no write attempted until after the phases; zero permission-denied in the console). Surfaced on matrix-arc-test re-extract of PRJ402063, 2026-07-09 (bar confirmed still stuck). FIX DIRECTION: timeout/abort + surface-or-skip a hung pricing call so a phase can't freeze the bar indefinitely. Possibly related to B013/B016 (on-open BC churn / token degradation). Distinct from B019 (that's the cosmetic bounce; this is a real hang). *(source: Jon P1 matrix, 2026-07-09; Marc + Freddy read.)*
