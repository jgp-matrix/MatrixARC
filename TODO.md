...
### 🐛 Bugs (B###)
- **B046 — "To-Do pane header shows 'To-Do List' (no first name) for some users (Andrew)"** [✅ FIXED on Test V.022 (2026-07-22) — userFirstName + memberMap now fall back to auth displayName→email-prefix when profile firstName unset; ships to prod next batch] — the F025 pane header `` userFirstName?`${userFirstName}'s To-Do List`:"To-Do List" `` (~:45200) falls to the bare label for Andrew → `userFirstName` is empty for him (works for Jon). Coach tracing the `userFirstName` source (member-doc `firstName`? auth displayName? owner-only path?) + why it's empty for a team member. Fix likely a fallback (displayName / email-prefix / member firstName) and/or populate firstName for all members. *(source: Jon relaying Andrew 2026-07-22.)*
