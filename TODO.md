...
- **G002 — "Automate the 4-session team startup boot"** [BUILT — pending calibration/live test] — boot the CCD team with minimal human interaction. Jon chose **Option A** (~95% automation, accept ~6 one-time Allow-Once clicks). **Marc delivered `1ba33d58`** (`tools/team-boot/`: `team-boot.ps1` PowerShell orchestrator + `lib/*.ahk` AutoHotkey v2 helpers + `onboarding/*.txt` 4 static blocks incl. Dez + README). `-WhatIf` dry-run validated end-to-end; honored the hard constraints (no auto-click Allow, launcher sets titles, paste-into-input). **NOT live-tested** — CCD-GUI specifics (new-session open, rename, timing) are calibration knobs needing ONE pass on Jon's desktop (set `$CcdExe`, confirm the GUI sequences, run → 4 titled+onboarded windows → comms-check). *(source: Jon 2026-07-02, via Freddy.)*
...
- **G001 — "Suppress the Allow-Once prompt on cross-session sends"** [Verified — NOT fixable; accepted limitation] — goal was to stop Jon being desk-tethered to approve the per-send "Allow Once" `send_message` prompt. *(source: Jon 2026-07-02, via Freddy; captured FEAT, reclassified G — dev-tooling/infra.)*
  ...
  - **PRACTICAL SEVERITY = LOW (key nuance, 2026-07-02).** The prompt is a **one-time approval per (sending session → target) pair**, remembered within that session — NOT per-message. Observed: 1st Freddy→Coach send prompted, 2nd went silent. So each session's startup costs ~one click per comm pair it uses (~6 total under hub-and-spoke: Freddy ↔ each of Coach/Marc/Dez), then the bus runs silent. A **session reboot resets its memory** → re-prompts once per pair after a refresh (why Coach's reset re-prompted). Not exhaustively stress-tested for re-prompts deep in a long session. Mitigations unchanged: batch sends; repo-commit handoff for unattended stretches; the stuck-on-Allow monitor stays useful.
