...
- **G001 — "Suppress the Allow-Once prompt on cross-session sends"** [Verified — RESOLVED] — original goal: stop Jon being desk-tethered to approve the per-send "Allow Once" `send_message` prompt. *(source: Jon 2026-07-02, via Freddy; captured FEAT, reclassified G — dev-tooling/infra.)*
  ...
  - **Decisive test (2026-07-02, confirmed via Coach reboot):** added the allow rule to `.claude/settings.json`, then refreshed Coach's session. Jon-observed result: a session that has **NOT reloaded** still prompts for un-approved targets (Freddy→new-Coach prompted; the earlier Freddy→Dez "no prompt" was a *remembered per-target approval*, NOT the rule — a red herring). A session that **HAS reloaded** (rebooted Coach) sends **prompt-free to a fresh target** → the `.claude/settings.json` rule genuinely suppresses the prompt, **but only after a session reload.** The "hardcoded/not-suppressible" lore was wrong.
  - **Resolution:** allow rule committed (`a1e786d3`); each session suppresses the prompt **once it reloads** to load the rule. **Rollout:** Coach ✅ (rebooted, confirmed prompt-free); Marc / Dez / Freddy adopt on their next reload. No remote-approval infra needed. **Supersedes** the SESSION-STATE/CLAUDE.md "hardcoded / do-not-re-test" note (CLAUDE.md correction routed to Coach). Stuck-on-Allow-block monitor stays useful until all four have reloaded.
