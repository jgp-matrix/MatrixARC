...
> **🆕 NEW 2026-07-27 (Jon, via Intake):** **B063 [BUG · LOW · not money-path]** — make the **BC Item Browser search case-insensitive**. `bcSearchItems` (`src/app.jsx:5011`) builds the server-side BC OData primary-token query as `contains(${field},'${primary}')` (`:5062`) with the ORIGINAL-case token → OData `contains()` is case-sensitive, so lower/mixed-case searches miss items. The client-side secondary-token filter already lowercases (`:5058+`) — only the server primary-token fetch is case-sensitive. Fix = case-fold the primary-token filter across the `ITEM_CARD_FIELDS` fan-out (`contains(tolower(${field}),'${primary.toLowerCase()}')`). Quick, isolated. Verify BC OData supports `tolower()` on these fields (it does for standard text fields) + that it doesn't defeat any index/perf.
