...
### ✨ Features (F###)
- **F003 — "Role-Differentiated Tech Review"** [Building — Jon-approved build 2026-07-06, Marc building] — User sees a checkbox→row turns YELLOW ("in review"), locked once sent for review; assigned Engineer (+admin) sees an empty GREEN CIRCLE (not the checkbox) only while `preReviewStatus="pending"`→signs off→row reverts + circle checked; Approve blocked until all yellow rows addressed (replaces auto-resolve sweep), Reject free. Absorbs C6 (no TR label) + C8 (yellow row, visual-only). Ruled set 1a/2b/3b/4a/5a/6b/keep/none. Brief: docs/F003-TECH-REVIEW-REDESIGN-BRIEF.md; Plan: docs/F003-COACH-BUILD-PLAN.md. *(source: Jon 2026-07-06, from F002 verify.)*
