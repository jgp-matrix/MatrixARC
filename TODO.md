...
### 🐛 Bugs (B###)
- **B047 — "ProjectTile owner/EDITING name overflows the tile at narrow width"** [OPEN · MED · 🔨 building (worktree)] — Jon (2026-07-22, screenshot at min tile width): the Row-1 right-side owner name / "<FIRST> EDITING" indicator runs OFF the right edge of the tile (worst under IN PROCESS + ACTIVE ECO, and with the "· ECO 01" label eating width + the longer "RYAN EDITING"). Fix: name must ALWAYS stay inside (right-justified, shrink/ellipsis, tile clips); long EDITING text wraps to a 2nd line ("EDITING" on line two). Presentation-only, scoped to ProjectTile Row 1. Building in a worktree (parallel to F031 #3, disjoint region → clean merge). *(source: Jon 2026-07-22.)*
