...
### 🔧 General (G###)
- **G001 — "Suppress the Allow-Once prompt on cross-session sends"** [Discovery — allowlist suppression NOT yet cleanly confirmed] — original goal: stop Jon being desk-tethered to approve the per-send "Allow Once" `send_message` prompt. *(source: Jon 2026-07-02, via Freddy; captured FEAT, reclassified G — dev-tooling/infra.)*
  ...
  - **Testing status (2026-07-02) — INCONCLUSIVE, confounded by per-target approval memory.** CCD remembers an "Allow Once" approval **per target, within a session**, so a *second* send to the same target is silent regardless of any allow rule. Observed (Jon): Freddy→new-Coach **first** send prompted; Freddy→new-Coach **second** send silent = remembered approval, NOT proof of the rule. The earlier Freddy→Dez "no prompt" was likewise a remembered approval (red herring). **We do NOT yet have a clean confirmation that the `.claude/settings.json` rule suppresses the prompt.**
  - **Clean test still needed:** observe a send from a freshly-REBOOTED session to a target it has NOT approved since reboot. Candidate datum: whether **Coach's first post-reboot outbound send (its reply to Freddy) prompted Jon** — un-confirmed (Jon's confirmed prompt was for Freddy's first comms-check; Coach's-reply prompt status not separately verified). Do NOT mark resolved until confirmed. Allow rule remains committed (`a1e786d3`) — harmless either way; the SESSION-STATE/CLAUDE.md lore correction still stands (the prompt IS at least sometimes avoidable; exact trigger TBD).
