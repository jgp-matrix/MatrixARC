...
- **B006 — "Scan Results 'concern' window is persistent with no relevant data"** [RESOLVED — shipped v1.22.2, 2026-07-06 · release `5cb5c392` (build `ab7c2309`)]
- **B007 — "TR checkbox still not rendering white (post-v1.22.2)"** [Open — diagnosing; Marc re-engaged] — after v1.22.1 (checked-white accentColor) + v1.22.2 (unchecked `appearance:none`+white bg), Jon reports the checkbox still isn't white. Marc DOM-verified the TR checkbox renders white on test + prod bundle → so likely NOT a missing fix. Diagnose: (a) is it the SEPARATE **budgetary checkbox** (accentColor amber `#f59e0b`, intentionally untouched)? (b) **stale cached bundle** (Fastly-fronted — hard-reload Ctrl+Shift+R)? (c) **faint 0.5-opacity-when-unflagged** reads greyish (Jon kept faint; if that's "not white," fix = full white / drop opacity). *(source: Jon 2026-07-06; Jon messaged Marc directly.)* — the `ScanResultsBanner` (app.jsx:23117) already hides at 0 concerns, but is dominated by benign auto-fixed INFO concerns (OCR-merged / audit-corrected / auto-retry), so a clean scan still shows the bar — hence "no relevant data." FIX: add a **"Dismiss" ✕** — **option B (per-project dismiss-by-report):** ✕ persists the dismissed report's identity (e.g. `panel.scanBannerDismissedTs = extractionReport.timestamp`, additive/data-safe + save), bar hides while it matches; a NEW extraction (new timestamp) auto-**resurfaces** it. Matches Jon's "hide now, surface later if relevant." Deferred deeper option (not chosen): auto-open only for ACTIONABLE concerns + quiet the benign. *(source: Jon 2026-07-06.)*
...
- **F001 — "Interactive quote-building walkthrough"** [Discovery · ⛔ blocked by F003] — gated hands-on training walkthrough (reusable engine + always-available Help button) that spotlights screen areas + gives concise directions for building a quote from scratch; targets the final BOM UI + F002/F003 `data-tour` anchors. Brief: docs/F001-QUOTE-WALKTHROUGH-BRIEF.md. *(source: Jon 2026-07-06.)*
...
- **G004 — "Tool-permission allowlist for suppressible prompts"** [Built — verify at G002 reboot] — pre-populate `.claude/settings.json` `permissions.allow` (committed → every new session inherits it) to cut the SUPPRESSIBLE (non-comm) tool prompts. *(source: Jon 2026-07-02.)* Safe entries only: read-only tools (Read/Grep/Glob), non-destructive git (status/diff/log/show/rev-parse/branch/add/commit/push/pull/fetch/stash), node (validate_jsx/`--check`), `pwsh`. Does NOT touch the `send_message` comm prompt (hardcoded — G001). **VERIFY at the G002 reboot:** sessions boot with it → confirm the target prompts (esp. Marc's PowerShell) actually stop; if `Bash(pwsh:*)` doesn't cover the PowerShell **tool** invocations, Marc adjusts the syntax. No destructive commands allowlisted.
