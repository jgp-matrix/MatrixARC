...
- [2026-07-09] FEAT — "Suggested alternates — multi-alternate 'ALT' picker (BC Browser) + supplier-portal alternate sub-lines" — extend the crossed/superseded feature to SUGGEST additional alternates under a BOM item (not just replace it). Two halves: (A) ARC BOM UI — a small "ALT" button next to the search icon on a row opens the BC Item Browser; user searches + selects one or more items into a "list" in the modal; the selected alternates are added to the Crossed/Supersede list in NOTES as BULLETED items UNDER the primary customer Part#, each with its Lead Time shown next to it. (B) Supplier Portal — where suppliers verify quote pricing + lead times, add a button next to each RFQ line item that lets the supplier add "alternate" option(s): it creates a SUB-LINE below the quoted item where the supplier fills in the alternate's Part#, Price, and Lead Time. Example: Item 15, Qty 1, KA2U — supplier offers 3 alternates with better pricing/lead time → shown as bullets under KA2U. Note: distinct from the existing single-part cross (which REPLACES the primary) — this ADDS suggested alternates alongside the primary. — reported via Intake (source: Jon)
