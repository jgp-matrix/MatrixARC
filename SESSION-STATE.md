...
**1. ★★ SHIP PHASE B — the PROVEN data-loss fix (B012). THIS is the first task.** Built + unit-tested **25/25** (`tests/bom-merge-phaseb.test.js`) + Coach **C138 PASS** + deployed to TEST + **PR #5**. Only step left: the **live 2-session matrix**. **Decisive test:** Jon + a **second real user** (edit-role — Andrew, or provision a test user) run `docs/PHASE-B-MATRIX-SCRIPT.md` on **matrix-arc-test** — load-bearing cases **T3** (deleted row NOT resurrected), **T7** (my unsaved add survives a remote save), **T9** (concurrent-add row keeps priceSource/metadata whole). **Marc CANNOT run it solo** (needs 2 distinct logins; his tab also can't reach the test host — Jon drives). **Pass → Jon prod sign-off → merge branch + deploy → relax the one-editor containment.** DONE = matrix green + prod deploy.
