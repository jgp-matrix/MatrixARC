...
4. **RFQ (Jon):** *"RFQs can go out, but they cannot be Accepted until BC is connected. Otherwise it will not populate pricing."* → **RFQ SEND is allowed when BC is down** (it needs no BC). But the **ACCEPT of a returned supplier quote is gated on BC** — the point of accept is to populate pricing, which needs BC.
   - **✅ RESOLVED (Jon 2026-07-27):** on Accept/Apply, the supplier prices **SAVE to ARC/Firestore locally** (never lost, per #3), but the **BC-writeback that populates pricing into BC QUEUES** and fires on reconnect — the accepted row is marked **"pending BC"** rather than the whole Accept being blocked. So: never lose the supplier's returned data; never push stale/down-state data to BC.

**F071 build unblocked** — B064 shipped (v1.24.41, exposes `_bcEndpointBroken()`) + all decisions locked incl. the RFQ/Accept resolution above.
