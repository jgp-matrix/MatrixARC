...
### WS3 — Fix the BOM planning-line 400 (independent unknown)
Root cause not yet known — the pilot was 429-throttled before the 400 body was captured. Candidates: item-not-found (master-data number mismatch), a required field/posting-group/location/dimension differing in the new sandbox, or the job/task ref.
- **Diagnose live** on the new sandbox in the fixed pilot: capture **one** `Project_Planning_Lines_Excel` 400 **response body** (the BC error message) to pin the exact rejection. (`bcFetchPurchasePrices` reads also 400'd — capture that body too; may share a root.) This step gates the rest of WS3.
- **Surface failures:** `relinkToBC` must check the planning-line sync `result.failed` (like B067) and report per-line failures + a non-✓ outcome — never "✓ Re-linked" on partial failure.
- **Back-off / fail-fast:** stop the sync after N consecutive BC errors (avoid the 429 "too many error requests" storm); respect 429 `Retry-After`.
