...
### F003 VERIFY COMPLETE + JON SIGN-OFF (2026-07-06)
**Full T1–T11 + all 4 Rev-A items PASS** (Marc live-verify, matrix-arc-test; results `docs/F003-VERIFY-RESULTS.md`, commit 77d52502). Build commits: `72c5994e` (core) + `dae43068` (Rev-A). Live-passed: role-mutual-exclusivity (Sales checkbox vs engineer circle), C6 no-label, Q2b lock, C8 yellow, engineer green-circle sign-off (ref-click → ✓ + audit stamp + row revert), approve-gate (disabled "Resolve N flagged…" → enabled after sign-off), Reject-free, bold 2px circle, sign-off final/uncheckable, "Issues" header. T7 (no sweep) + T8 (send-gate) code-verified (T8 also live-passed at the earlier F003 T2). **Jon signed off the brighter yellow `rgba(250,204,21,0.4)` (#facc15).**
**Next:** Coach final review (diff + results) → **Jon deploy checkpoint** (F003 ships; likely MINOR bump v1.22.0 — new workflow capability).
**Loose ends (G005, test data):** PRJ402111 carries 4 resolved TR flags + `preReviewStatus="pending"` from the passes — reset if reused.
