...
**4. Auto tear-off (`$AutoTearOff` + `tearoff-session.ahk`, commit `6bd80e63`) — SAFE; no-op guard is AIRTIGHT (verified for click-misfire).** Two independent layers, both confirmed:
- **PS1 layer:** `$AutoTearOff` defaults `$false`, so the tear-off block (lines 153–164) never executes and the helper is **never invoked** in the default configuration.
- **AHK layer (the guarantee Freddy asked to confirm):** `Calibrated := false` → `if (!Calibrated) ExitApp(9)` is the **first executable statement** in `tearoff-session.ahk`, positioned **before** `WinExist` / `WinActivate` / `MouseMove` / `Click` / any `Send`. The only side-effecting code (window activation + the ⋯-menu sequence) sits *after* the guard, and the entire "⚙ calibrate from here" block is **commented-out pseudo-code**. So while uncalibrated there is **literally zero reachable window-activation, mouse-move, click, or keystroke** — no aim, no misfire is possible.
- **Degrade path:** if someone flips `$AutoTearOff=$true` without calibrating, the AHK `ExitApp(9)` propagates via `& $AhkExe` (Write-Host does not clobber `$LASTEXITCODE`) → PS1 line 158 detects `-eq 9` → warns "not calibrated — skipping" + `break` → falls back to manual tear-off. Clean, no partial action.

Wired correctly, not dangling. **The exit-9/no-op guard genuinely prevents any click misfire while uncalibrated — confirmed.**
