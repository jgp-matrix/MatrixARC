...
- **`leaseReadOnly`** — ONE added disjunct on the composite `readOnly` (@37349):
  ...
  ```
  **Note on per-session:** the RULES are per-**uid** (rules only see `request.auth.uid`, not a tab). So the same user's 2nd tab is made read-only **client-side** (both tabs share the uid, so rules would allow either — the client's `editingTabId` compare is what stops the 2nd tab). This is acceptable: the same user is not an adversary, so client enforcement closes the self-clobber residual; the *cross-user* guarantee (the actual B012 case) is server-enforced.
...
- **P1 — Core lock + read-only (the data-safety fix).** Lease fields; `editingTabId`; claim/heartbeat(tab-open)/release; rules `editingLeaseValid`/`isEditingLeaseLocked`/`isOnlyLeaseUpdate` (with P2/P3 helpers stubbed) added to `allow update`; `leaseReadOnly` disjunct + read-only banner; 3 save guards; new/copy/restore scrub; Async-Ownership hold-while-bg-task; per-session client tab compare. **Ships the guarantee.** MEDIUM.
