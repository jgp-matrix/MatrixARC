...
> **SUPPLEMENT REVIEWED + RULINGS (2026-07-06)** — `docs/F001-COACH-SUPPLEMENT.md`, **Analyst Review: PASS.**
> ★ **A tour engine ALREADY EXISTS** (`TourOverlay`, app.jsx:47927, shipped v1.19.667 — narrated tours + localStorage resume + gear-menu launcher + ~14 anchors). **F001 EXTENDS it; does NOT build new — supersedes Brief §3a.** Delta = GATED auto-advance + **click-through spotlight** (the main risk) + CHECKPOINT state-waits + state-driven resume. All lifecycle triggers have located signals (extraction `activeExtractions`, supplier `pendingRfqUploads`, TR `preReviewStatus`, sent `quoteSentAt` — no new backend). Re-entrancy = HYBRID (state picks phase = self-healing; existing localStorage picks sub-step; NO new Firestore field). ~15–19 anchor adds across ~6–7 components. Lift MEDIUM; two-track build (engine vs anchors), step array last.
> **★ JON DATA-SAFETY RULING: run on the user's REAL project — NO training sandbox** (drop Supplement §5.1 option-b bootstrap). **The two consequential SEND steps — 4Ba Send-RFQs + Step-7 Send-Quote — are NARRATED, NOT gated:** the tour explains + points at the Send button but never auto-fires or gate-detects the send; the user performs the real send on their own timing (downstream checkpoints 4Bb / Step-6 then resume off real project state). Keeps it real-work-authentic while preventing the guided run from misfiring a supplier/customer email.
> **Next: Coach writes the build Plan** (exact `TourOverlay` diffs + click-through gating + checkpoint watches + state-driven resolver + full anchor list + the 7-step array with per-step types, 4Ba/Step-7 = narrated) → Analyst Review → Jon approve → Marc builds (two tracks).
>
> **BUILD PLAN APPROVED (2026-07-06)** — `docs/F001-COACH-BUILD-PLAN.md` (tip 4ab48692), **Freddy Analyst Review PASS** (extends `TourOverlay` backward-compatibly — existing tours safe; both Jon rulings encoded; Step 5 correctly gated as internal; risk isolated to A3 click-through + A5 wiring). **Jon APPROVED the build** → Marc builds **Track A (engine)** ∥ **Track B (~15–19 anchors)** → **Track C (7-step array; 4Ba+Step7 NARRATED)** → `validate_jsx` → HOLD. Then live-verify (click-through landing, checkpoint resume mid-lifecycle, narrated sends never auto-fire, existing tours unaffected, a11y) → Coach review → Jon deploy checkpoint. Gating: code-complete ≠ deploy.
