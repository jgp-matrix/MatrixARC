...
function Badge({ status }: any) {
  return <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 6, background: C.border, color: C.muted }}>{status}</span>;
}
...
export default function App({ user }: { user: any }) {
  ...
  useEffect(()=>{
    if(!user.uid)return;
    const unsub=(fbDb as any).collection(`users/${user.uid}/notifications`).where('read','==',false).orderBy('createdAt','desc').limit(50).onSnapshot((snap: any)=>{
  ...
  useEffect(()=>{
    if(!user.uid)return;
    const unsub=(fbDb as any).collection('rfqUploads').where('uid','==',user.uid).where('status','==','submitted').onSnapshot((snap: any)=>{
      const counts: any={};
  ...
  function handleNotifClick(notif: any){
    (fbDb as any).doc(`users/${user.uid}/notifications/${notif.id}`).update({read:true}).catch(()=>{});
  ...
  function markAllNotifsRead(){
    notifications.forEach((n: any)=>(fbDb as any).doc(`users/${user.uid}/notifications/${n.id}`).update({read:true}).catch(()=>{}));
  }
...
- If you don't have the specific standard text in ARC Neural IQ, say so and provide your best knowledge with a note that the user should verify against the actual standard.`;
  ...
  useEffect(()=>{
    (fbDb as any).collection('users').doc(user.uid).collection('config').doc('preferences').get()
      .then((snap: any)=>{
        if(snap.exists){
    ...
    const checkBc=async()=>{
      ...
      try{
        ...
        if(r.status===401){
          ...
          setBcToken(null);bcOnlinePrev.current=false;setBcOnline(false);setBcLostAlert(true);return;
        }
        const ok=r.ok;
        if(ok&&!bcOnlinePrev.current){
          setBcOnline(true);
          // BC just came back online — refresh company info and process queue
          bcFetchCompanyInfo().then((info: any)=>{
            if(info&&_appCtx.companyId){
              ...
              if(info.address||info.phone||info.name){
                (fbDb as any).doc(`companies/${_appCtx.companyId}`).update({...(info.name?{name:info.name}:{}),address:info.address||"",phone:info.phone||""}).catch(()=>{});
              }
            }
          }).catch(()=>{});
          bcProcessQueue();
  ...
  useEffect(()=>{
    ...
    const apply=(s: string)=>setConnStatus((prev: string)=>prev!==s?s:prev);
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(profile?.companyId){
        ...
        try{
          const cd=await (fbDb as any).doc(`companies/${profile.companyId}`).get();
          if(cd.exists){
            const cdat=cd.data();
            ...
            _appCtx.company={name:cdat.name||null,logoUrl:cdat.logoUrl||null,address:cdat.address||null,phone:cdat.phone||null};
      ...
      (fbDb as any).doc(`users/${user.uid}/config/profile`).get().then((d: any)=>{if(d.exists)setUserFirstName(d.data().firstName||"");}).catch(()=>{});
      // Load company members → uid→name map for project ownership display
      if(profile?.companyId){
        loadCompanyMembers(profile.companyId).then(async (mems: any)=>{
          const map: any={};
          const profileDocs=await Promise.all(mems.map((m: any)=>(fbDb as any).doc(`users/${m.uid}/config/profile`).get().catch(()=>null)));
      ...
      if(!_defaultBomItems.length){
        ...
        setDefaultBomItems(defaults);
        saveDefaultBomItems(user.uid,defaults).catch(()=>{});
      }
      ...
      if(!_bcToken){acquireBcToken(false).then(async (t: any)=>{if(t){console.log("BC auto-connected silently");setBcOnline(true);bcOnlinePrev.current=true;bcProcessQueue();setProjects((ps: any)=>[...ps]);}}).catch(()=>{});}
    })();
  },[user.uid]);
  ...
  function checkQuoteRevWarn(action: ()=>void){
  ...
  function handleOpen(p: any){
    checkQuoteRevWarn(()=>{setRevSnoozed((s: any)=>{const n={...s};delete n[openProject?.id];return n;});setOpenProject(p);setView("project");});
  }
  function handleCreated(p: any){setShowNew(false);setProjects((ps: any[])=>[p,...ps]);setOpenProject(p);setView("project");}
  function handleChange(p: any){setProjects((ps: any[])=>ps.map((x: any)=>x.id===p.id?p:x));setOpenProject(p);}
  function handleDelete(id: string,name: string,bcProjectId?: string,bcProjectNumber?: string,project?: any){setDeleteConfirm({id,name,bcProjectId,bcProjectNumber,project});}
  async function confirmDelete(deleteFromBC: boolean){
  ...
  function handleTransferDone(id: string){
  ...
  async function handleAccept(id: string){
    ...
    await (fbDb as any).doc(`${path}/${id}`).update({
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      ...
      {splash!=="done"&&(
        <div style={{position:"fixed",inset:0,zIndex:9999,background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",
            ...
            <svg width="100" height="90" viewBox="0 0 100 90" fill="none" style={{marginBottom:12}}>
              <polygon points="50,4 96,86 4,86" stroke={C.accent} strokeWidth="3.5" strokeLinejoin="round" fill={C.accentDim}/>
              <polygon points="50,4 68,38 32,38" fill={C.accent} opacity="0.85"/>
              <line x1="50" y1="38" x2="50" y2="86" stroke={C.accent} strokeWidth="1.2" opacity="0.35"/>
              <line x1="26" y1="62" x2="74" y2="62" stroke={C.accent} strokeWidth="1.2" opacity="0.35"/>
            </svg>
            ...
            <span style={{fontSize:15,color:C.muted,letterSpacing:2,marginBottom:32}}>by <span style={{background:"#ef4444",color:"#fff",borderRadius:20,padding:"2px 14px",fontWeight:700,letterSpacing:1}}>RedPill</span> Software</span>
          </div>
          {splash==="loading"&&(
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,zIndex:500}}>
        {/* Main toolbar row */}
        <div style={{background:C.card,borderBottom:`1px solid ${C.border}`,padding:"0 24px",display:"flex",alignItems:"center",height:78,gap:6,position:"relative"}}>
          {/* RedPill logo — left side */}
          <img src="/redpill_logo.png" alt="RedPill Software" style={{height:48,objectFit:"contain",cursor:"pointer",flexShrink:0}} onClick={()=>checkQuoteRevWarn(()=>{setRevSnoozed((s: any)=>{const n={...s};delete n[openProject?.id];return n;});setView("dashboard");setOpenProject(null);})}/>
          ...
          <div style={{position:"relative",flexShrink:0}}>
            ...
            {showBellMenu&&(<div style={{position:"absolute",right:0,top:"calc(100% + 6px)",background:"#0d0d1a",border:`1px solid ${C.border}`,borderRadius:10,padding:"6px 0",minWidth:320,maxWidth:380,boxShadow:"0 8px 30px rgba(0,0,0,0.8)",zIndex:600}}>
              ...
              <div style={{maxHeight:360,overflowY:"auto"}}>
                {notifications.length===0&&<div style={{padding:"20px 16px",textAlign:"center",color:"#475569",fontSize:13}}>No new notifications</div>}
                  ...
                  <div key={n.id} style={{padding:"10px 16px",borderBottom:`1px solid ${C.border}22`,cursor:n.type==='supplier_quote'?"pointer":"default"}}
                    ...
                    <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:10,color:"#475569",marginTop:3}}>{n.createdAt?new Date(n.createdAt).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}):""}</div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {/* 👤 User menu */}
          <div style={{position:"relative",flexShrink:0}}>
            <div data-tour="team-btn" title={user.email} onClick={()=>{setShowUserMenu((v: boolean)=>!v);setShowGearMenu(false);}} style={{width:44,height:44,borderRadius:"50%",background:showUserMenu?"#1e3a5f":C.border,border:showUserMenu?`2px solid ${C.accent}`:`2px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:18,fontWeight:800,color:showUserMenu?C.accent:"#94a3b8",flexShrink:0,userSelect:"none",letterSpacing:0}}>
              {user.email?user.email[0].toUpperCase():"?"}
            </div>
            {showUserMenu&&(<div style={{position:"absolute",right:0,top:"calc(100% + 6px)",background:"#0d0d1a",border:`1px solid ${C.border}`,borderRadius:10,padding:"6px 0",minWidth:230,boxShadow:"0 8px 30px rgba(0,0,0,0.8)",zIndex:600}}>
      ...
      {/* Content — padded below fixed header, with side panel */}
      <div style={{flex:1,paddingTop:78,display:"flex",height:"100vh",overflow:"hidden"}}>
      <div style={{flex:1,minWidth:0,overflowY:"auto",transition:"margin-right 0.3s ease"}}>
      ...
      {showSettings&&<SettingsModal uid={user.uid} onClose={()=>setShowSettings(false)} onNameChange={(n: string)=>setUserFirstName(n)}/>}
      {showReports&&<ReportsModal uid={user.uid} onClose={()=>setShowReports(false)}/>}
      {showConfig&&<PricingConfigModal uid={user.uid} onClose={()=>setShowConfig(false)} onLogoChange={(url: string)=>{setCompanyLogo(url||null);_appCtx.company={...(_appCtx.company||{}),logoUrl:url||null};}}/>}
      ...
      {showSetup&&<CompanySetupModal uid={user.uid} email={user.email} onDone={(cid: string,role: string,name: string)=>{setCompanyId(cid);setUserRole(role);setCompanyName(name||null);setShowSetup(false);}} onClose={()=>setShowSetup(false)}/>}
      {revWarnModal&&(()=>{const rm=revWarnModal;const rev=rm.project.quoteRev||0;return React.createElement("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"},onClick:(e: any)=>{if(e.target===e.currentTarget){rm.pendingAction();setRevWarnModal(null);}}},
        React.createElement("div",{style:{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"32px 36px",maxWidth:440,width:"90%",textAlign:"center"}},
          ...
          React.createElement("div",{style:{display:"flex",flexDirection:"column" as const,gap:10}},
            ...
            React.createElement("button",{onClick:()=>{rm.pendingAction();setRevWarnModal(null);},style:{background:(C as any).surface,color:C.muted,border:`1px solid ${C.border}`,borderRadius:20,padding:"8px 20px",fontSize:13,fontWeight:600,cursor:"pointer"}},"Dismiss")
          )
        )
      );})()}
                  ...
                  {msg.role==='user'
                    ...
                    :<div style={{fontSize:13,color:"#cbd5e1",lineHeight:1.7}} dangerouslySetInnerHTML={{__html:(()=>{
                      let html=msg.content
                        ...
                        .replace(/^\d+\. (.+)$/gm,(m: string,p1: string)=>'<div style="padding-left:12px">'+m.match(/^\d+/)![0]+'. '+p1+'</div>')
                        .replace(/\n{2,}/g,'<br/><br/>')
                        .replace(/\n/g,'<br/>');
                      return html;
                    })()}}/>
