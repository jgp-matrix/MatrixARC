...
export default function QuoteView({project,uid,onBack,onUpdate}: any){
  ...
  const allBom=mergeBoms(panels.map((p: any)=>p.bom||[]));
  const totalWireCount=panels.reduce((s: number,p: any)=>s+(p.validation?.wireCount||0),0);
  // Compute labor for each panel individually (preserves overrides), then sum
  const perPanelLabor=panels.map((p: any)=>computeLaborEstimate(p));
  const totalLaborHours=perPanelLabor.reduce((s: number,l: any)=>s+l.totalHours,0);
  const totalLaborCost=perPanelLabor.reduce((s: number,l: any)=>s+l.totalCost,0);
  const hasLayoutData=perPanelLabor.some((l: any)=>l.hasLayoutData);
  // Merge labor lines by category
  const laborLineMap: any={};
  perPanelLabor.forEach((l: any)=>l.lines.forEach((line: any)=>{
  ...
  const firstPanelPricing=panels.find((p: any)=>p.pricing)?.pricing||{};
  ...
  const aggregated={
    ...
    pages:panels.flatMap((p: any)=>p.pages||[]),
  ...
  function handleQuoteUpdate(upd: any){
    // Sync pricing back to all panels so panel cards stay consistent with quote
    const updatedPanels=(project.panels||[]).map((p: any)=>({...p,pricing:{...(p.pricing||{}),...(upd.pricing||{})}}));
          ...
          {panels.length>0&&(
            <div style={{...card({padding:"12px 16px"}),marginBottom:20}} className="no-print">
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
                {panels.map((p: any)=>{
                  const mats=(p.bom||[]).reduce((s: number,r: any)=>s+(r.unitPrice||0)*(r.qty||1),0);
                  return(
                    <div key={p.id} style={{...card({padding:"10px 14px"}),minWidth:170}}>
                      ...
                      {p.complianceReview?.concerns?.length>0&&(()=>{const cc=p.complianceReview.concerns;const crit=cc.filter((c: any)=>c.severity==='critical').length;const warn=cc.filter((c: any)=>c.severity==='warning').length;return <div style={{fontSize:11,marginTop:2}}>{crit>0&&<span style={{color:'#ef4444',fontWeight:700,marginRight:4}}>{crit} critical</span>}{warn>0&&<span style={{color:'#f59e0b',fontWeight:600}}>{warn} warning{warn>1?'s':''}</span>}</div>;})()}
