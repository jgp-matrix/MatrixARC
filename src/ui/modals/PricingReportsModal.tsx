...
function PricingReportsModal({uid,onClose}){
  ...
  return(<div style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.7)"}} onClick={onClose}>
    <div style={{background:C.card,borderRadius:12,border:`1px solid ${C.border}`,width:800,maxWidth:"95vw",maxHeight:"85vh",display:"flex",flexDirection:"column",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderBottom:`1px solid ${C.border}`}}>
        <div style={{fontSize:15,fontWeight:700,color:C.text}}>Pricing Sync Reports</div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {runs.length>0&&<button onClick={exportAllLatest} style={{background:C.green,color:"#fff",border:"none",borderRadius:6,padding:"6px 14px",fontSize:11,fontWeight:600,cursor:"pointer"}}>Export All Latest Prices (CSV)</button>}
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 18px"}}>
        ...
        :<div>
          {runs.map(run=>{
            ...
            return(<div key={run.id} style={{marginBottom:8,background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,overflow:"hidden"}}>
              {/* Run summary row */}
              <div onClick={()=>setExpandedId(expanded?null:run.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",cursor:"pointer",flexWrap:"wrap"}}>
                <span style={{fontWeight:700,color:run.vendor==="Codale"?C.teal:C.purple,fontSize:12,minWidth:60}}>{run.vendor}</span>
                <span style={{color:C.muted,fontSize:12}}>{dt.toLocaleDateString()} {dt.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</span>
                <span style={{color:C.green,fontSize:12,fontWeight:600}}>{run.found} found</span>
                {run.errors>0&&<span style={{color:C.red,fontSize:12,fontWeight:600}}>{run.errors} errors</span>}
                <span style={{color:C.muted,fontSize:12}}>{run.totalItems} total</span>
                {run.writtenToBC>0&&<span style={{color:C.accent,fontSize:12}}>→ {run.writtenToBC} written to BC</span>}
                <span style={{color:C.muted,fontSize:11}}>{fmtDuration(run.durationMs)}</span>
                <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center"}}>
                  <button onClick={e=>{e.stopPropagation();exportRunCSV(run);}} style={{background:C.border,color:C.text,border:"none",borderRadius:4,padding:"4px 10px",fontSize:11,cursor:"pointer",fontWeight:600}}>CSV</button>
              ...
              {expanded&&<div style={{borderTop:`1px solid ${C.border}`,padding:"8px 14px",maxHeight:400,overflowY:"auto"}}>
                ...
                :(run.results||[]).map((r,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"4px 0",borderBottom:`1px solid rgba(255,255,255,0.03)`,fontSize:12,flexWrap:"wrap"}}>
                  <span style={{fontWeight:700,color:r.found?C.green:C.red,minWidth:140}}>{r.partNumber}</span>
                  {r.found?<>
                    <span style={{color:C.green,fontWeight:700,minWidth:80}}>${Number(r.price).toLocaleString("en-US",{minimumFractionDigits:2})}</span>
                    <span style={{color:C.muted}}>{r.uom||"EA"}</span>
                    {r.manufacturer&&<span style={{color:C.muted}}>| {r.manufacturer}</span>}
                    {r.productName&&<span style={{color:C.muted,fontSize:11}}>{r.productName}</span>}
                  </>:<span style={{color:C.muted}}>{r.error||"Not found"}</span>}
                </div>)}
              </div>}
            </div>);
          })}
