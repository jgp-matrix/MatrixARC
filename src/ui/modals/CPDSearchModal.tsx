...
function CPDSearchModal({query,uid,panel,onClose,onImportBom}: any){
  ...
  const [quoteAnalysis,setQuoteAnalysis]=useState<any>(null); // {quote,results,totalPanels,matchedCount}
  const [marginPct,setMarginPct]=useState(30);

  const [similarPanels,setSimilarPanels]=useState<any[]>([]);

  // Find similar panels from CPD database
  useEffect(()=>{
    loadCPD(uid).then((cpd: any)=>{
      const panels=cpd.panels||[];
      if(!panels.length)return;
      const q=query.toLowerCase();
      const scored=panels.map((p: any)=>{
        let score=0;
        const haystack=((p.panelType||'')+(p.controlledEquipment||'')+(p.panelName||'')+(p.additionalNotes||'')).toLowerCase();
        q.split(/\s+/).forEach((word: string)=>{if(word.length>2&&haystack.includes(word))score++;});
      ...
      }).filter((p: any)=>p._score>0).sort((a: any,b: any)=>b._score-a._score).slice(0,3);
  ...
  useEffect(()=>{
    ...
    async function run(){
      try{
        ...
        (cpd.products||[]).forEach((p: any)=>{topCategories[p.category]=(topCategories[p.category]||0)+1;});
        const panelCount=(cpd.panels||[]).length;
        const panelSummaries=(cpd.panels||[]).slice(-5).map((p: any)=>`${p.panelType||'Panel'} for ${p.controlledEquipment||'unknown equipment'} (${p.specs?.motorCount||0} motors, PLC I/O: ${p.specs?.plcInputs||0}in/${p.specs?.plcOutputs||0}out, ${p.voltages?.lineVoltage||'unknown voltage'})`).join('; ');
...
Use realistic part numbers (Allen-Bradley, Phoenix Contact, Schneider, etc.) where confident. Set unitPrice to 0 if unknown.`;
        ...
        if(cancelled)return;
        const raw=(resp as any).content[0].text.trim();
        const json=raw.replace(/^```json\s*/,'').replace(/^```\s*/,'').replace(/\s*```$/,'');
        const rows=JSON.parse(json);
        const bomRows=rows.map((r: any,i: number)=>({
          id:`cpd-${Date.now()}-${i}`,
        ...
        setResult(bomRows);
        setSelected(new Set(bomRows.map((r: any)=>r.id)));
        setPhase("result");
      }catch(e: any){
  ...
  useEffect(()=>{
    ...
    async function run(){
      try{
        ...
        const quotes=snap.docs.map((d: any)=>({id:d.id,...d.data(),_ms:d.data().importedAt?.toMillis?.()||0}))
          .sort((a: any,b: any)=>b._ms-a._ms);
        const q=quotes[0];
        const quoteItems=(q.lineItems||[]).filter((qi: any)=>qi.isPriced&&(qi.partNumber||qi.rawPartNumber));
        const bomRows=(panel.bom||[]).filter((r: any)=>!r.isLaborRow&&(r.partNumber||'').trim());
        ...
        const results=bomRows.map((row: any)=>{
        ...
        const matched=results.filter((r: any)=>r.panelsFromItem!==null);
        const totalPanels=matched.length>0?Math.min(...matched.map((r: any)=>r.panelsFromItem)):null;
        if(cancelled)return;
        setQuoteAnalysis({
      ...
      }catch(e: any){if(!cancelled){setErrorMsg(e.message);setPhase('error');}}
    }
    run();
    return()=>{cancelled=true;};
  },[]);
  ...
  async function downloadExcel(){
    ...
    const salePriceTotal=results.reduce((s: number,r: any)=>s+(r.unitPrice!=null?r.unitPrice/(1-margin):0),0);
    dataRows.push({
      'Part #':'',
      'Description':'\u2500\u2500 TOTAL \u2500\u2500',
      'Unit Price':null,
      ...
      <div style={{background:"#0d0d1a",border:`1px solid ${C.border}`,borderRadius:12,width:"100%",maxWidth:isQuoteMode?1100:780,maxHeight:"88vh",display:"flex",flexDirection:"column",boxShadow:"0 12px 60px rgba(0,0,0,0.8)"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 24px"}}>
          {similarPanels.length>0&&(
            <div style={{marginBottom:14}}>
              ...
              {similarPanels.map((p: any,i: number)=>(
                <div key={p.panelId||i} style={{background:"#120a2a",border:"1px solid #a78bfa44",borderRadius:7,padding:"9px 12px",marginBottom:6}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                    ...
                    {p.controlledEquipment&&<span style={{fontSize:10,color:C.muted}}>&mdash; {p.controlledEquipment}</span>}
                  </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,background:C.bg,borderRadius:8,padding:"8px 14px",border:`1px solid ${C.border}`}}>
                  ...
                  <input type="number" min={0} max={99} value={marginPct}
                    onChange={(e: any)=>setMarginPct(Math.max(0,Math.min(99,Number(e.target.value)||0)))}
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:700}}>
                    <thead>
                      <tr style={{background:C.card,color:C.muted,borderBottom:`2px solid ${C.border}`}}>
                        {['Part #','Description','Unit Price','BOM Qty','Qty Quoted','Panels','Margin %','Sale Price'].map((h: string)=>(
                ...
                {unmatchedCount>0&&(
                  <div style={{fontSize:11,color:C.yellow,background:"#1a0d00",border:`1px solid ${C.yellow}33`,borderRadius:6,padding:"8px 12px"}}>
                    &#9888; {unmatchedCount} BOM item{unmatchedCount!==1?'s':''} had no matching line in the supplier quote &mdash; shown as &ldquo;&#8212;&rdquo; for quoted qty and panels buildable.
                  </div>
                )}
              </div>
            );
          ...
          {phase==="result"&&result&&(
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <div style={{fontSize:12,color:C.muted}}>{result.length} items generated &mdash; check the items you want to import</div>
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>setSelected(new Set(result.map((r: any)=>r.id)))} style={{fontSize:11,background:"none",border:`1px solid ${C.accent}`,borderRadius:4,color:C.accent,padding:"3px 8px",cursor:"pointer"}}>All</button>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {result.map((r: any)=>{
                  ...
                  return(
                    <div key={r.id} onClick={()=>setSelected((prev: Set<string>)=>{const s=new Set(prev);checked?s.delete(r.id):s.add(r.id);return s;})}
        ...
        {phase==="result"&&(
          <div style={{padding:"14px 24px",borderTop:`1px solid ${C.border}`,display:"flex",gap:10,justifyContent:"flex-end",flexShrink:0}}>
            ...
            <button onClick={()=>onImportBom(result.filter((r: any)=>selected.has(r.id)))}
