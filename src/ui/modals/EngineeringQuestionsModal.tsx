...
function EngineeringQuestionsModal({panel,uid,onUpdate,onSave,onClose,memberMap}){
  ...
  const sevColors={critical:C.red,warning:C.yellow,info:C.purple};
  const statusLabels={open:"Open",answered:"Answered",skipped:"Skipped",on_quote:"On Quote"};
  const statusColors={open:C.yellow,answered:C.green,skipped:C.muted,on_quote:C.accent};
  ...
  return ReactDOM.createPortal(
    React.createElement("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:9998,display:"flex",alignItems:"center",justifyContent:"center",padding:16},onMouseDown:e=>{if(e.target===e.currentTarget)onClose();}},
      React.createElement("div",{style:{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"20px 24px",maxWidth:620,width:"95%",maxHeight:"85vh",display:"flex",flexDirection:"column",boxShadow:"0 4px 20px rgba(0,0,0,0.12)"}},
        React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10,marginBottom:12,flexShrink:0}},
          ...
          React.createElement("span",{style:{fontSize:10,fontWeight:700,color:C.yellow,background:C.yellowDim,borderRadius:10,padding:"2px 8px"}},openCount+" open"),
          answeredCount>0&&React.createElement("span",{style:{fontSize:10,fontWeight:700,color:C.green,background:C.greenDim,borderRadius:10,padding:"2px 8px"}},answeredCount+" answered"),
          skippedCount>0&&React.createElement("span",{style:{fontSize:10,fontWeight:700,color:C.muted,background:C.bg,borderRadius:10,padding:"2px 8px"}},skippedCount+" skipped"),
          onQuoteCount>0&&React.createElement("span",{style:{fontSize:10,fontWeight:700,color:C.accent,background:C.accentDim,borderRadius:10,padding:"2px 8px"}},onQuoteCount+" on quote")
        ),
        React.createElement("div",{style:{overflowY:"auto",flex:1,marginBottom:12}},
          ...
          eqs.map(q=>React.createElement("div",{key:q.id,style:{marginBottom:10,padding:"10px 14px",background:q.status==="open"?"rgba(250,204,21,0.04)":"rgba(255,255,255,0.02)",border:`1px solid ${q.status==="open"?"rgba(250,204,21,0.2)":"rgba(255,255,255,0.06)"}`,borderRadius:8}},
            ...
            q.status==="answered"?
              React.createElement("div",{style:{fontSize:12,color:C.green,fontWeight:600,padding:"4px 8px",background:C.greenDim,borderRadius:4}},"Answer: ",q.answer):
            q.status==="on_quote"?
              React.createElement("div",{style:{fontSize:12,color:C.accent,fontWeight:600,fontStyle:"italic"}},"Included on customer quote"):
            q.status==="skipped"?
            ...
            React.createElement("div",null,
              ...
              React.createElement("input",{value:answers[q.id]||"",onChange:e=>setAnswers(a=>({...a,[q.id]:e.target.value})),placeholder:"Type your answer…",
                style:{width:"100%",background:C.input,border:`1px solid ${C.border}`,color:C.text,borderRadius:4,padding:"5px 8px",fontSize:11,outline:"none",boxSizing:"border-box",marginBottom:6}}),
              React.createElement("div",{style:{display:"flex",gap:6,flexWrap:"wrap"}},
                React.createElement("button",{onClick:()=>{if(!answers[q.id])return;updateQ(q.id,{answer:answers[q.id],answeredBy:uid,answeredAt:Date.now(),status:"answered"});},
                  disabled:!answers[q.id],style:{fontSize:11,color:C.green,background:answers[q.id]?C.greenDim:"transparent",border:`1px solid ${C.green}44`,borderRadius:4,padding:"3px 10px",cursor:answers[q.id]?"pointer":"default",opacity:answers[q.id]?1:0.4,fontWeight:600}},"Answer"),
                React.createElement("button",{onClick:()=>updateQ(q.id,{status:"skipped"}),style:{fontSize:11,color:C.muted,background:"none",border:`1px solid ${C.border}`,borderRadius:4,padding:"3px 10px",cursor:"pointer"}},"Skip"),
                React.createElement("button",{onClick:()=>updateQ(q.id,{status:"on_quote"}),style:{fontSize:11,color:C.accent,background:"none",border:`1px solid ${C.accent}44`,borderRadius:4,padding:"3px 10px",cursor:"pointer"}},"Include on Quote")
              )
            )
          ))
        ),
        React.createElement("div",{style:{display:"flex",gap:8,justifyContent:"space-between",alignItems:"center",flexShrink:0,borderTop:`1px solid ${C.border}`,paddingTop:12}},
          React.createElement("button",{onClick:()=>setEmailPicker(true),style:{fontSize:12,color:C.purple,background:"none",border:`1px solid ${C.purple}44`,borderRadius:6,padding:"6px 14px",cursor:"pointer",fontWeight:600}},"Email Controls Engineer"),
        ...
        emailPicker&&React.createElement("div",{style:{marginTop:10,padding:"10px 14px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8}},
          ...
          React.createElement("div",{style:{display:"flex",gap:6}},
            React.createElement("input",{value:emailTo,onChange:e=>setEmailTo(e.target.value),placeholder:"email@company.com",
              style:{flex:1,background:C.input,border:`1px solid ${C.border}`,color:C.text,borderRadius:4,padding:"5px 8px",fontSize:11,outline:"none"}}),
