...
function SupplierPricingUploadModal({uid,onClose}: any){
  ...
  const [colMap,setColMap]=useState<any>({partNumber:-1,description:-1,unitCost:-1});
  const [reviewRows,setReviewRows]=useState<any[]>([]);
  const [checked,setChecked]=useState<any>({});
  const [lookupProgress,setLookupProgress]=useState<any>(null);
  ...
  function parseCSV(text: string){
    ...
    for(const line of lines){
      ...
      const row: any[]=[];let cur='';let inQ=false;
  ...
  function handleFile(f: any){
    ...
    reader.onload=(e: any)=>{
      ...
      // Auto-detect columns
      const map: any={partNumber:-1,description:-1,unitCost:-1};
      h.forEach((col: any,i: number)=>{
  ...
  async function runLookup(){
    ...
    const rows: any[]=[];const seen=new Set();
    for(let i=0;i<rawRows.length;i++){
      ...
      if(seen.has(pn.toUpperCase())){
        const existing=rows.find((x: any)=>x.partNumber.toUpperCase()===pn.toUpperCase());
    ...
    setReviewRows(rows);
    const chk: any={};rows.forEach((_: any,i: number)=>{chk[i]=true;});setChecked(chk);
    setLookupProgress(null);
  }

  async function runBatch(){
    setPhase('processing');
    const toProcess=reviewRows.filter((_: any,i: number)=>checked[i]);
    const prog: any={total:toProcess.length,current:0,updated:0,created:0,errors:[]};
    setProgress({...prog});
    if(!_bcToken){try{await acquireBcToken(true);}catch(e){}}
  ...
  const overlay: any={position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:700,display:"flex",alignItems:"center",justifyContent:"center"};
  const modal: any={background:C.card,borderRadius:14,width:900,maxWidth:"96vw",maxHeight:"85vh",display:"flex",flexDirection:"column",border:`1px solid ${C.border}`,boxShadow:"0 8px 40px rgba(0,0,0,0.6)"};
  const hdr: any={padding:"18px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"};
  const body: any={flex:1,overflowY:"auto",padding:"20px 24px"};

  return(
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={(e: any)=>e.stopPropagation()}>
        <div style={hdr}>
          <div style={{fontSize:16,fontWeight:800,color:C.text}}>{"\u{1F4E5} Upload Supplier Pricing"}</div>
        ...
        {phase==='upload'&&(<>
          ...
          <div
            ...
            <input ref={fileRef} type="file" accept=".csv" style={{display:"none"}} onChange={(e: any)=>{const f=e.target.files[0];if(f)handleFile(f);}}/>
            <div style={{fontSize:40,marginBottom:12}}>{"\u{1F4E5}"}</div>
        ...
        {phase==='mapping'&&(<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:20}}>
            {([['Part Number','partNumber',true],['Description','description',false],['Unit Cost','unitCost',true]] as any[]).map(([label,key,req]: any)=>(
              <div key={key} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:12}}>
                ...
                <select value={colMap[key]} onChange={(e: any)=>setColMap((prev: any)=>({...prev,[key]:parseInt(e.target.value)}))}
                  ...
                  {headers.map((h: any,i: number)=><option key={i} value={i}>{h}</option>)}
                </select>
              </div>
            ))}
          </div>
          ...
          <div style={{overflowX:"auto",borderRadius:8,border:`1px solid ${C.border}`,marginBottom:16}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <thead><tr style={{background:"#0a0a12"}}>{headers.map((h: any,i: number)=><th key={i} style={{padding:"6px 10px",textAlign:"left",color:i===colMap.partNumber?C.accent:i===colMap.unitCost?C.green:i===colMap.description?C.yellow:C.muted,fontWeight:700,whiteSpace:"nowrap",borderBottom:`1px solid ${C.border}`}}>{h}</th>)}</tr></thead>
              <tbody>{rawRows.slice(0,5).map((row: any,ri: number)=><tr key={ri}>{row.map((cell: any,ci: number)=><td key={ci} style={{padding:"5px 10px",color:C.text,borderBottom:`1px solid ${C.border}22`,whiteSpace:"nowrap"}}>{cell}</td>)}</tr>)}</tbody>
            </table>
          </div>
        ...
        {phase==='review'&&!lookupProgress&&(<>
          <div style={{fontSize:13,color:C.muted,marginBottom:12}}>
            <strong style={{color:C.text}}>{reviewRows.filter((r: any)=>r.status==='update').length}</strong> items to update, <strong style={{color:C.accent}}>{reviewRows.filter((r: any)=>r.status==='new').length}</strong> new items to create. Uncheck rows to skip them.
          </div>
          <div style={{overflowX:"auto",borderRadius:8,border:`1px solid ${C.border}`,marginBottom:16,maxHeight:400,overflowY:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:"#0a0a12",position:"sticky",top:0,zIndex:1}}>
                <th style={{padding:"8px 10px",width:36}}><input type="checkbox" checked={reviewRows.every((_: any,i: number)=>checked[i])} onChange={(e: any)=>{const v=e.target.checked;const c: any={};reviewRows.forEach((_: any,i: number)=>{c[i]=v;});setChecked(c);}} style={{accentColor:C.accent}}/></th>
              ...
              <tbody>{reviewRows.map((row: any,i: number)=>{
                ...
                return(
                  <tr key={i} style={{borderBottom:`1px solid ${C.border}22`,opacity:checked[i]?1:0.4}}>
                    <td style={{padding:"6px 10px",textAlign:"center"}}><input type="checkbox" checked={!!checked[i]} onChange={(e: any)=>setChecked((prev: any)=>({...prev,[i]:e.target.checked}))} style={{accentColor:C.accent}}/></td>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={runBatch} disabled={!Object.values(checked).some((v: any)=>v)}
              style={{flex:1,padding:"10px 20px",background:C.accent,border:"none",borderRadius:8,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",opacity:Object.values(checked).some((v: any)=>v)?1:0.5}}>
              {"Apply "+Object.values(checked).filter((v: any)=>v).length+" Items to BC \u2192"}
            </button>
          </div>
        </>)}
