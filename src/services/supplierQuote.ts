...
// Extracted from monolith index.html lines ~3001-3153.
...
function sqExtractMfr(raw: any): { mfr: string; partNumber: string } {
...
function sqNormalize(raw: any): any {
 ...
 */
export function sqValidateLineItems(items: any[]): any {
  const dupeLines: any[] = [], dupeParts: any[] = [];
  const lnMap: any = {}, pnMap: any = {};
  ...
  Object.entries(lnMap).filter(([, a]: any) => a.length > 1).forEach(([ln, idxs]: any) => dupeLines.push({ ln: Number(ln), rows: idxs }));
  Object.entries(pnMap).filter(([, a]: any) => a.length > 1).forEach(([, idxs]: any) => dupeParts.push({ pn: items[idxs[0]].rawPartNumber, rows: idxs }));
...
export async function saveSupplierQuoteToFirestore(
  quoteData: any,
  userId: string,
  { supersedes = null, projectId = null, bcProjectNumber = null }: any = {}
  ...
  // Build search tokens for array-contains queries (supplier, quote#, job name, contact)
...
export async function sqGetAiPrior(uid: string): Promise<number> {
  try {
    const snap = await db().collection('users').doc(uid).collection('config').doc('sqLearning').get();
    ...
    return Math.max(10, Math.round(avg * 1.15)); // rolling avg + 15% buffer
  } catch { return 22; }
}
...
export async function sqRecordAiTime(uid: string, seconds: number): Promise<void> {
  try {
    const ref = db().collection('users').doc(uid).collection('config').doc('sqLearning');
...
export async function sqGetCrossings(uid: string): Promise<any> {
  try {
    const snap = await db().doc(`users/${uid}/config/sqCrossings`).get();
...
export async function sqSaveCrossing(uid: string, supplierPN: string, bcItem: any): Promise<void> {
  ...
  await db().doc(`users/${uid}/config/sqCrossings`).set(
...
export async function sqGetVendorMap(uid: string): Promise<any> {
  try {
    const snap = await db().collection('users').doc(uid).collection('config').doc('supplierVendorMap').get();
    return snap.exists ? snap.data() : {};
  } catch { return {}; }
}
...
export async function sqSaveVendorMapping(uid: string, supplierName: string, vendorNo: string): Promise<void> {
  try {
    ...
    await db().collection('users').doc(uid).collection('config').doc('supplierVendorMap').set({ [key]: vendorNo }, { merge: true });
...
export async function sqSavePushAudit(quoteDocId: string, supplier: string, quoteId: string, userId: string, auditItems: any[]): Promise<void> {
  ...
  await db().collection('bcPriceUpdates').add({
...
export function sqFuzzyMatchVendor(supplierName: string, vendors: any[]): any {
  ...
  for (const v of vendors) {
    ...
    // If first words match exactly, score gets a strong 0.6 base boost
...
/**
 * Derives a 3-letter supplier code from the vendor name for RFQ numbering.
