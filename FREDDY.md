...
**Last updated:** 2026-08-07 (EOD)
**Also works for:** Mid-session reorientation after context compaction. Paste again if Freddy loses context.

> **Current (2026-08-05 EOD):** prod **v1.24.96**, `master==origin` @ `04a7f924`, tree clean, **NOT frozen**. Marathon BC-integrity + status-system day. Shipped v1.24.91→v1.24.96: **B097** (false "unsynced/must-be-priced" modal — predicate SSOT align), **B099+B091** (Refresh & Apply reliably reloads via cache-bust `location.replace(?_cb=)` + timeout-raced SW teardown; downgrade-guard so the version modal only prompts for a strictly-newer build — live-verified via a reversible red-dot Test crossing), **B100** (Copy Project carries ALL drawing assets — hydrate source from Firestore + copy native PDFs + remap; verified PRJ402509 16/16 pages), **B098** (dash-agnostic BC surrogate resolve via alphanumeric-run anchors + FIRM-MFR collision-breaker), **B102** (crossed rows re-bind bcNo to the crossed-TO part, never push the original). **B104** (price-edit revert on RAPID edits) root-CONFIRMED via live instrumentation (stale-closure/last-write-wins save race) → Coach-scoped build plan `docs/B104-save-race-fix-plan.md`, **deferred to a fresh session (Jon)** = the #1 next item. **B101** status-flag-sequencer spec FINALIZED (`docs/B101-kanban-status-rules-review.md` §6d, Jon-confirmed). **B102 remediation:** BC-verified 61 wrong crossed rows / 32 projects (heuristics unreliable — BC resolution is truth), not yet run. Intake captured (INBOX): B103 (BUYOFF auto-add config), B105 (Item Browser dash-search=F082), B106 (vendor-name ingress), F091 (fix-BC-part#-propagate). **#1 open: build B104.** Full ranked NEXT UP in SESSION-STATE.md + STATUS.md.
...
- **Current version:** **v1.25.7** (prod, 2026-08-07 EOD; freeze lifted early — Jon directed 7 releases this session). **★ 2026-08-07 big ship day (F089 v1.25.0 → v1.25.7 + B106 200-row vendor data repair):** F089 (Refresh Pricing+LT — Jon-verified live on PRJ402509) · **B106** vendor name≠number repair COMPLETE (148 repoint + 49 restamp + 3 Hoists = 200 rows, all reversible `docs/b106-backfill-logs/`, final REPOINT/RESTAMP/AMBIGUOUS all 0; V00251=Heitek de-mislabeled; reconcile = STOP per Jon, F089-Refresh proven on PRJ402509 L1) · B108/G029 (Upload-SQ modal: Accept-cross shows Vendor Part#, removed Add-to-BOM-Only) · B110/B112 (RFQ tile "N of M RFQs RCVD" pill → now HONEST, real supplier submissions not priced-ness; PRJ402143="0 of 8") · G030 (hide redundant Ready-To-Send pill) · B111 (RFQ History "Received"=truly-submitted only; pending stay ⏳ Awaiting) · **#85** (Excel/CSV → BOM direct import, shipped w/ Coach M1 owner-priority gate). **NEXT UP: F098** (Quote Line# + B065-Phase-2 durable ARC↔BC binding — build-ready design `docs/F098-quote-line-binding-plan.md`, 4 Jon decisions, forward-only) is #1; then B109 (vendor find/replace corrupted descriptions — needs triage), F096 (vendor create-dedup), F090 (BOMv scoped); G028 tabled; F097 parked on Test V.083 (F098 subsumes). Prior day: **2026-08-06 shipped 13 items v1.24.98→.105** (B101+B096 / F092 / F093 / G024-026 / F094 / B107 / F095 / B105). ——— HISTORICAL (pre-2026-08-06) ——— ★ (v1.24.53 era) BC pointed at the UAT sandbox `MATR_SndBx_UAT_070926` (Sales in UAT). The 2026-07-29 session shipped **v1.24.52** (RFQ-send stale-Priced-Date hotfix, F075 #2) + **v1.24.53** (F075 Phase 2 — Get-Prices Confirmed/Budgetary gate + opt-in record-as-optional; #1/#3 primary-overwrite fixed). **▶ OPEN: Vendor Sync mapping blocked → resume AM** (need `digikeyVendorNo=V00196`/`mouserVendorNo=V00304` in `users/{uid}/config/vendorConfig`; no clean UI, direct write classifier-blocked; see STATUS.md "▶ RESUME HERE"). Prior context ↓. ★ The 2026-07-24 session shipped a **big batch (v1.24.34→v1.24.36)**: F065 cross-Line propagation + **full-sync** [Update all] (price+LT+vendor), B060 (BC-circle flicker), B058 (circle membership-driven), B061 (Ext$ width/format), F066 (Duct/DinRail/DuctCover red only on $0), F067 (3-min auto-approve), G019/G020/G021; **F068** (cross-propagation) built + Coach-approved + on Test (branch `claude/f068-cross-propagation`, pending Jon cross re-verify → prod); **B032** re-diagnosed → RESOLVED (stale tag). Prior 2026-07-23 session = PRJ402119 pricing-incident firefight (v1.24.13→v1.24.33). **CRITICAL: pricing is RFQ-ONLY now — auto-pricing/scraper/AI-estimate are intentionally OFF via kill-switches (~app.jsx:5506-5518); do NOT "fix" the disabled pricing** (memory `project_rfq_only_pricing_mode`). Shipped: containment kill-switches, B052 (poll divergence guard), F045 (budgetary mgr-gate), F057 (CS display), F058 (pill removals), F059 (Mark Committed), F060 (To-Do/Dashboard restructure), F061 (ECO column removal), F051j (BC junk-price cleanup), B057 (write-path supersede/expire), F044 (block-send-on-red + mgr override), F046/F047 (price-setter stamp + hover), B056 (trailing-dot boot guard), B055 (BC Item Browser overflow), G018 (Debug Logs date filter), G019 (Questions hidden behind flag), F063 (SHOW SENT toggle + Dashboard column), F064 (ECO-to-top). Extraction model is **Claude Opus 4.8**. See SESSION-STATE.md for the full current picture + open items.
...
## Recently Active Work (as of 2026-08-03)

### ★ LATEST — 2026-08-05 session (subagent-lane model; prod v1.24.90 → v1.24.96) — BC-integrity + status-system marathon; B104 root-confirmed + deferred
Shipped 6 prod releases, all Coach-reviewed + live-verified: **B097** (false "Unsynced changes to BC / must be priced" modal — aligned `_panelUnpricedForBc` to the RED-row SSOT `_isExcludedFromPriceCheck`), **B099+B091** (the "Refresh & Apply" version modal now reliably reloads: timeout-race the SW/caches teardown + cache-busting `location.replace(?_cb=Date.now())` so a plain reload can't re-serve a stale/intermediate index.html; + `_versionIsNewer` guard + monotonic `_system/version` write so it never prompts a DOWNGRADE — verified via a reversible red-dot Test-build crossing), **B100** (Copy Project drops drawings → now hydrates the source fresh from Firestore + copies native original PDFs + remaps paths; the caller passed a dashboard-lite object so pages arrived ref-less), **B098** (`_bcResolveSurrogateExact` dash-AGNOSTIC: after the raw contains() search, fall back to maximal alphanumeric-RUN anchors → matches when BC's dash placement differs, e.g. ARC `6ES7511-1AL03-0AB0` vs BC `6ES75111AL03-0AB0`; + FIRM ≥3-char MFR collision-breaker), **B102** (crossed rows re-bind bcNo to the crossed-TO part — auto-replace/supplier-portal cross paths spread the stale ORIGINAL surrogate + a set bcNo blocked the re-resolve → planning-line push sent the wrong part; fix CLEARS bcNo/bcVerify so it re-resolves). **B104** (price edits revert on RAPID entry — recurring, days-spent): root CONFIRMED by live in-tab instrumentation (captured a stale `saveProjectPanel` writing the prior value 99ms after the newest write) = stale-closure / last-write-wins save race; `saveProject`+`saveProjectPanel` share no lock + no value-level stale-write guard. Coach-scoped fix (fire-time-latest saves + monotonic per-panel `_localEditSeq` guard + shared save lock = correctness core of deferred B016 Fix B) in `docs/B104-save-race-fix-plan.md`; **Jon deferred the build to a fresh focused session** = #1 next. **B101** status-flag-sequencer: authoritative spec finalized (Jon-confirmed 6-rung dynamic ladder + §6e lifecycle audit stamps), NOT built. **B102 remediation:** BC-verified 61 wrong crossed rows / 32 projects (upper-bound heuristics said 387→605; only resolving bcNo→BC Vendor_Item_No gave truth — memory `feedback_verify_prevalence_against_authoritative_source`). Intake logged to INBOX (un-promoted, per Jon: next-session triage): B103/B105/B106/F091. Process lessons this session (memories): `feedback_stay_ahead_anticipate_constraints` (know test-env potholes — G008 Storage-CORS + TEST_BUILD label collision — BEFORE sending Jon to test), `feedback_verify_prevalence_against_authoritative_source`, `reference_partnum_dash_normalize_mfr_secondary`. Detail: SESSION-STATE.md + STATUS.md + docs/B098/B100/B101/B102/B104.

### 2026-08-03 session (subagent-lane model; prod v1.24.68 → v1.24.83 + functions + firestore:rules) — full bug queue + F086 broadcast/forced-refresh shipped
Root-caused + fixed Ryan's extraction save-loss (PRJ402141): an 18-page add flooded the Firestore client write-queue (`resource-exhausted`) and the extraction save error was swallowed → BOM silently lost. Shipped the whole staged queue, all Coach-reviewed: **B078-1** (retry + blocking modal, no silent green ✓), **B078-2** (per-project background-save COALESCER = the root fix, + airtight stale-clobber guard), **B079** (unique panel names via `panelSeq`), **B081** (Auto-Add config shows vendor part#), **B080** (functions: Anthropic 429 retry-with-backoff, deadline-aware, +CONCURRENCY 4→3, +latent `pageResults` fix), **B082** (Margin/sell-price PATCH flush-on-leave), **F085** (BC-sync-on-leave prompt). Then built **F086** (Admin Global Msg + version-broadcast forced-refresh, folds F008) with heavy live UX iteration: centered TAKEOVER modal (must-ack), hard-reload (unregister SW + clear caches) — the stale-bundle fix behind Ryan's problem, `version.json` 60s poll + instant `_system/version` ping, "Later"→4-min snooze-nudge, admin-locked broadcast rule S1-pinned to company `XODxZ8xJc0dQXGZI7jbo` (Jon test-verified). **"3-user issues" = the B078 save-loss (Jon confirmed, RESOLVED).** **#1 open:** verify the F086 4-min nudge live + team one-time Ctrl+Shift+R to bootstrap onto ≥v1.24.83. **LOW backlog:** F085 F1/F2, B080 F1/F2 (.gcloudignore + supplier-404 body-drain), B083; S1 multi-tenant hardening (future). Detail: SESSION-STATE.md + STATUS.md + docs/B078-*, docs/F085-*, docs/F086-*, docs/B080-*, docs/B081-*.

### 2026-07-29 session (subagent-lane model; prod v1.24.51 → v1.24.53) — BC-sandbox UAT + F075 Get-Prices Phase 2; Vendor Sync mapping BLOCKED (resume AM)
Context: BC now points at the new UAT sandbox `MATR_SndBx_UAT_070926` (Sales in UAT). **Shipped prod:** **v1.24.52** — RFQ-send stale-Priced-Date **hotfix** (F075 finding #2: strip stale `bcPoDate` on API apply so `_effectivePriceDate` shows the fresh date; Coach APPROVE-WITH-NITS; Jon Test-verified). **v1.24.53** — **F075 Phase 2** (the 📥 Get Prices button reworked around Jon's Confirmed/Budgetary model): Budgetary/empty/stale rows get the API price on-row **keeping the existing supplier** (fixes findings #1/#3 — no more DigiKey overwrite) + strip `bcPoDate`; Confirmed rows are protected + offered in an **opt-in confirm modal** to record the API vendor's price as an OPTIONAL BC Purchase Price (F072 machinery; F071 BC-down HARD-BLOCK + $0 guard; vendor# from `config/vendorConfig`; UoM via `bcGetItemBaseUom`). Stale-Confirmed = KEEP PROTECTED (Jon). Coach APPROVE-WITH-NITS (UoM nit folded). Jon Test-verified #2/#3 live, #1 code-verified. Manual-price reframe (Jon): the **Confirmed-vs-Budgetary** prompt (DECISION v1.19.376) is the human-intent gate — the old "never overwrite manual" rule is retired for this flow. **▶ OPEN BLOCKER (Jon deferred to morning):** activating the optional-BC-write needs `digikeyVendorNo`/`mouserVendorNo` in `users/{uid}/config/vendorConfig` (verified DigiKey=`V00196`, Mouser=`V00304`) — but no clean way to set them (only the mass "Update All DigiKey Prices" sync writes them; Mouser has no field; direct Firestore write classifier-blocked). ⇒ Phase 2 optional-write is STRANDED until a small "map API vendor → BC vendor #" config UI exists (= the queued **"New-API-source BC-vendor check"** / RS-Online item, now a prerequisite). 3 options in STATUS.md "▶ RESUME HERE" (rec: build the small config UI). **Parked:** F076 (portal manual price/LT entry); `1002` non-MTX item in PRJ402119 (Step-2 reconcile leftover, needs Jon BC-item pick). Process note: live prod controlled-tab probes confirmed the vendor #s + config gap; browser-injected mutations (and function-source reads) are classifier-blocked — app-mediated UI only. Detail: STATUS.md + `docs/F075-PHASE2-BUILD-PLAN.md`.

### 2026-07-24 session (subagent-lane model; prod v1.24.33 → v1.24.36) — F065 full-sync + F066/F067 + display batch shipped; F068 on Test
Shipped prod (all Jon-verified, money-path Coach-reviewed): **F065** cross-Line Part# propagation + **full-sync** [Update all] (price+LT+vendor in one click; Bug A override-manual w/ per-Line disclosure, Bug B index self-heals on open, Bug C confidence-pill clears) · **B060** `applyConfirmedPrice` promote-on-success (BC-circle flicker) · **B058** BC circle membership-driven (price-independent) · **B061** Ext$ colgroup width 64→108 + `$`-left format · **F066** Duct/DinRail/DuctCover RED only on $0/qty=0 (staleness + estimated-LT + LT-RFQ exempt via `_isReorderCommodity`/`_hasFirmLeadTime`) · **F067** 3-min auto-approve · **G019/G020/G021**. **B032** re-diagnosed → **RESOLVED** (stale EMERGENCY tag — fix was already in prod since v1.23.12). **F068** (propagate the cross A→B across Lines, full-sync) BUILT + Coach code-APPROVED + LT-timing fix, on branch `claude/f068-cross-propagation` / **Test V.055** — ⏳ pending Jon **CROSS re-verify** → merge to prod. **TABLED (Jon):** the lead-time/BC-Item-Browser cluster (**B062** — browser shows no LT + re-select doesn't push LT) = one lead-time-flow pass next session. Process lessons: money-path features run branch→Test→verify→prod (master stays prod-clean); Coach money-path review caught a real per-Line disclosure gap in F065 full-sync + a vendor-number mismatch in F068 (both fixed pre-ship); Jon's live test beat the static review on the F068 LT-timing (runtime-artifact-over-code-read). Detail: SESSION-STATE.md ⭐ NEXT UP + docs/F065-* + docs/F068-*.

### 2026-07-23 session (subagent-lane model; prod v1.24.13 → v1.24.33) — PRJ402119 pricing-incident firefight + big feature batch
