...
> **★ DEFAULT MODEL UPDATE (Jon 2026-07-12) — [drafted by Freddy in solo mode; Coach to curate/fold in]:** The **subagent-lane model (`/ARC-team-Startup`, renamed 2026-07-14 from `/team-sub-start`) is now Jon's preferred default.** Freddy runs in CCD with full repo access and runs the work as **in-session subagent lanes labeled by role** (Marc = build/fix, Coach = review/diagnose; announce the role on every spawn), is the sole git-writer + notifier, and **owns Dez's files (`STATUS.md`/`INBOX.md`) directly** (no Dez lane). Deploys stay a Jon gate; high-stakes/data-safety changes still get a Coach review + a live verification gate before prod. The standing 4-session model described below (`/team-startup`) still applies when Jon wants the full peer cross-check — but the subagent-lane model is the default. Details: FREDDY.md "★★ CURRENT OPERATING MODEL" + `.claude/commands/ARC-team-Startup.md` + memory `feedback_subagent_lane_model_preferred`.
...
| `/ARC-team-Startup` (renamed 2026-07-14 from `/team-sub-start`) | Freddy only (Marc/Coach/Dez = in-session subagent lanes) | Subagent variant of `/team-startup`: one Freddy session, no peer sessions / no bus / no Allow-Once prompts. Away-mode-native; Freddy = sole git-writer + sole notifier. Live browser + high-stakes builds still escalate. |
